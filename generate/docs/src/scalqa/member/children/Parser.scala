...
class Parser(owner: Member):
  ...
  extension(x: Member)    private def _child_?               : Opt[Member] = x.signature.?.drop(v => v.isEmpty || !v.head.isInstanceOf[Link]).map(_.head.asInstanceOf[Link].dri.moduleId)
                                                                             .takeOnly(owner.id.moduleId + "." + x.name.nameToId).map_?(Docs.module_?).map(_.main)
  // -----------------------------   ----------------------------------------------------------------------------------------------------------------------
  ...
  this.all = owner.members.~.dropOnlyBy(_.name,"THIS_TYPE"/*,"NumericOps","OrderingOps"*/).drop(m => Docs.isHiddenName(owner,m.name))
  owner.name match
  ...
  case "scalqa" =>
    ...
    aliases  = { members.~.drop(_.name=="J").flatMap(_.members).drop(v => v.name == "~" && v.kind.isDefLike) ++ all.~.find_?(_.name == "~~")
              ++ Docs.member_?(Member.Id("scalqa.gen.request$")).get.members }.sort
  case _ =>
