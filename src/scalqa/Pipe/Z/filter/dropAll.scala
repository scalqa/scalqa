...
private[Pipe] object dropAll {
  ...
  private def applyBy[A, B](src: Pipe[A], ssOpt: Opt[Sorting[B]], fun: A => B, filter: Pipe[B])(implicit srt: Sorting[B]): Pipe[A] =
    if (filter.sizeOpt.contains(0)) src
      ...
      object SortedMerge extends The.Build[A] {
        ...
        override def pumpOpt(f: A ⇒ Boolean): Opt[A] = {
          ...
          if (!fo) src.pumpOpt(f)
          else fo.letMap(_ => src.pumpOpt(a => {
            val v = fun(a)
            if (fo.let(srt(_, v).isAsIs)) { fo = filter.pumpOpt(srt(_, v).isNotAsIs); oo = fo }
            !fo.contains(v) && f(a)
          }))
        }
