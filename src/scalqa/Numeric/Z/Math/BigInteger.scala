...
private[Numeric] object BigInteger extends Math[JBig] {
  ...
  def apply(v: Int) = JBig.valueOf(v.toInt)
  def apply(v: Long) = JBig.valueOf(v.toInt)
  def apply(v: Double) = JBig.valueOf(v.toInt)
  def apply(v: Numeric) = v.I.letAs(classOf[BigInteger]).map(_.real) or JBig.valueOf(v.toLong)
