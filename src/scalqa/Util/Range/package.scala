...
package object Range {
  ...
  @inline def startContains[A](r: Range[A], v: A, x: Boolean = false) = Z.Base.startContains(r, v, x)
  @inline def endContains[A](r: Range[A], v: A, x: Boolean = false) = Z.Base.endContains(r, v, x)
}
