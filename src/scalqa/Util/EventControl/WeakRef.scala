...
object WeakRef {
  ...
  def apply[A](v: A) = make(v)

  def make[A](v: A) = new WeakRef[A](new java.lang.ref.WeakReference[A](v))

  implicit def zzTargetType[A](v: WeakRef[A]): A = v.real.get.Opt or (throw CancelException)
 ...
 *
 *   Note. [[WeakRef]] does not have any methods itself, but it is implicitly converted to target type,
 *   so it supports all the methods of target
 *
 *   {{{
 ...
 *        .onValueChange(t => {
 *          val mc : MyCounter = Util.EventControl.WeakRef.make(hardRef)
