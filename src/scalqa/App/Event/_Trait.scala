...
trait _Trait {
  ...
  def onCancel(b: () => Any): Event

  def onCancelRun(f: => Any): Event = onCancel(() => f)

  def cancelIf(b: () => Boolean): Event

  def cancelIfTrue(b: => Boolean): Event = cancelIf(() => b)

  def cancelIfFalse(b: => Boolean): Event = cancelIf(() => !b)

  def expireIn(tl: Duration): Event = { val t = Time.now + tl; cancelIf(() => Time.now > t) }

  def limitRunsTo(maxRunCount: Int): Event = if (maxRunCount <= 0) { cancel; this } else { var c = 0; cancelIf(() => maxRunCount < { c += 1; c }) }

}

object _Trait extends Util.Void.Setup[Event](Z.Void)
 ...
 * @trait _Trait ->
 *
 *    [[App.Event]] allows to cancel the event job directly or conditionally
 *
 *    {{{
 *       val timer: Pro.O[Time] = Time.currentPro(1.Second)               // Value changes every 1 second
 *
 *       val e1: App.Event = timer.onValueChange("1: " + _.subSeconds lp) // Prints seconds within minute
 *       val e2: App.Event = timer.onValueChange("2: " + _.subSeconds lp) // Prints seconds within minute
 *       val e3: App.Event = timer.onValueChange("3: " + _.subSeconds lp) // Prints seconds within minute
 *
 *       e1.limitRunsTo(2)
 *       e2.cancelIfTrue(e1.isCancelled)
 *       e2.onCancelRun(e3.cancel)
 *
 *       // Output
 *       3: 20 secs
 *       2: 20 secs
