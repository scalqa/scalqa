...
trait WithId {
  ...
  override def toString = "App.Event.Id" + (this match {
    case f: (() => Any) => "0"
    case f: ((_) => Any) => "1"
    case f: ((_, _) => Any) => "2"
    case f: ((_, _, _) => Any) => "3"
 ...
 *
 *    Most events in Scalqa are lazily cancelled through App.Event
 *
 *    However it is also possible to use traditional listener subscription model
 *
 *    To do so we have to create [[App.Event]] with some id when subscribing, and
 *    send cancellation [[App.Event]] with the same id to remove the subscription
 *
 *    {{{
 ...
 *
 *      val cc2 = pro.onValueChange(App.Event.get1[DayTime](listenerId, v => "New:" + v lp))
 *
 *      App.runIn(3.Seconds, {
 *        "removing event by id".lp
 *         pro.onValueChange(App.Event.getCancel1(listenerId))
 *      })
 *
