...
private[Extra] class _Class(chld: Entry, private var _scope: AnyRef, protected[Entry] var _this: Any) extends java.util.concurrent.atomic.AtomicReference[_Class](chld)
    with Target.Container with Extra.Entry with App.Event with Any.Able.Void {
  ...
  def cancel: Boolean = if (isCancelled) false else { var b = false; synchronized { if (!isCancelled) { _scope = null; b = true } }; if (b) Target.Wrap.fireOnCancel(_this); b }

  def all(scope: AnyRef): ~[Entry] = stream(this, scope)

  def toInfo = new Pro.Info(this) += ("scope", if (scope.Class.isSingleton) scope.Class else scope) += ("value", value) +~= isCancelled.toOpt(("", "isCancelled")).all
