...
trait _Trait extends Like {
  ...
  protected[Extra] override def _addEntry(scope: AnyRef, value: Any): Concurrent.Control = super._addEntry(scope, value)

  protected[Extra] def _isNoEntries: Boolean = _topEntry.isVoid

  protected[Extra] def _entriesDescription: String = _topEntry.all(Z.Entry.all.AnyScope).map(_.I.info).zipIdx.nonEmptyOpt.map(_.format("\n").Text.indent("Any.Extra: ")) or "empty"
 ...
 * @trait _Trait ->
 *
 *     [[[Any.Extra]]] is a universal mechanism of extending any object with all kinds of data. It is heavily used in Scalqa, particularly for event support.
 *
 ...
 *
 *     Using [[[Any.Extra]]] is easy:
 *     {{{
 *       // Two independent Int and String containers sharing same Any.Extra storage
 *
 *       class MyInts extends Any.Extra {
 *         def add(i: Int)       = _addEntry(MyInts.Scope, i)
 *         def remove(i: Int)    = _allEntries(MyInts.Scope).let(_.value == i).apply(_.cancel)
 *         def allInts           = _allEntries(MyInts.Scope).map(_.value).mapCast[Int]
 *       }
 ...
 *         def allStrings        = _allEntries(MyStrings.Scope).map(_.value).mapCast[Int]
 *       }
 ...
 *    }}}
 *
 * @def _allEntries -> Enries pipe
 *
 *     Provides pipe of entries only for the specified scope.
 *
 * @def _addEntry -> Creates entry
 *
 *     Adds new entry to the list
 *
 *     Returns [[Concurrent.Control]], a fancy interface to remove entry.
 *
