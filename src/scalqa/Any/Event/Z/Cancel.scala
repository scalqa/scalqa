...
private[scalqa] class Cancel[A, B, C, D](val id: Any) extends Event with (() => Any) with ((Any) => Any) with ((A, B) => Any) with ((A, B, C) => Any) with ((A, B, C, D) => Any) {
  ...
  private def error = Fail.state(
    "The remove request was done on wrapped event. \n" +
      "Check the $anonfun few lines down.  \n" +
      "The wrapping was done without movid id to the new function.  \n" +
      "Any.Event.Id.moveId must be used when wrapping events")
}
