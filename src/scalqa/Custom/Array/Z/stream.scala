...
private[scalqa] object stream {
  ...
  @inline def apply[@specialized(DATA) A](arr: Array[A], len: Int = -1): Stream[A] with Any.Able.Stream[A] = new Stream[A] with Stream.A.Specialized.Indexed[A] with Any.Able.Stream[A] {

    @inline val _size = if (len >= 0) len else arr.length

    @inline def _apply(i: Int): A = arr(i)
