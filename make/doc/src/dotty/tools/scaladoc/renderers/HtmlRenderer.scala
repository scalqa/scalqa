  ...
  def render(): Unit = // scalqa start ----------------------------------------------------
    ...
    def prepare(page: Page, parents: Vector[Link]): Seq[(String,()=>Unit)] =
      ...
      def run: Unit = page.content match
        ...
        case _ =>
          val pg = page.^.reviseIf(_.link.dri.isOpaqueBase, p => { val d = p.link.dri.trimOpaqueBase; Page(Link(d.lastName + "!",d),Doc.member_?(d.memberId).get, Nil) })
  ...
  private def mkFrame(link: Link, parents: Vector[Link], content: => AppliedTag): AppliedTag =
    ...
    div(id := "container")(
      div(id := "leftColumn")(
        ...
        div(id := "paneSearch"),
        nav(id := "sideMenu2")(
          scalqa.Doc.Menu.build(this,navigablePage,link)
        ),
      ),
      div(id := "main")(
        div (id := "leftToggler")(
