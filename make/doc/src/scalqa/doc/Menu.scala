...
import dotty.tools.scaladoc.renderers.*
  ...
  def build(l: Locations, topPage: Page, link: Link): AppliedTag =
    ...
    def renderNested(page: Page): (Boolean, AppliedTag) =
      ...
      page.children.filter(_.link.name!="Dotty") match
        ...
        case children =>
          val nested = children.map(p =>if(p.link.name=="API") renderApi(Doc.rootModule) else renderNested(p))
    ...
    //renderApi(Doc.rootModule)._2

    div(Seq(cls := "expanded"))(
      renderNested(topPage.children.find(_.link.name=="Guide").get)._2,
      renderApi(Doc.rootModule.children.~.read)._2
    )
