package scalqa; package fx; package ui; package javaFx; package z; import language.implicitConversions
...
private[fx] class IdxO[A](val real: JFX.ObservableList[A]) extends Idx.O[A]:
  ...
  def onChange[U](l: ><[Idx.O.Event[A]] => U): Gen.Event.Control =
    if (l.isInstanceOf[Gen.Event.Id]) J.unsupportedOperation()
      ...
      object Listener extends Gen.Event.Control.X.Basic(l) with JFX.ListChangeListener[A] { def onChanged(real: JFX.ListChangeListener.Change[_ <: A]) = target_?.forval(_(Ui.JavaFx.As.parseChanges(real))) }
