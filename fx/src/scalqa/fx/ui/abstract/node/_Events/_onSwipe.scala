...
transparent trait _onSwipe:
  ...
  def onSwipeDown [U](l: Ui.Event.Swipe => U) : Gen.Event.Control = onSwipe(l, real.onSwipeDownProperty)
  def onSwipeLeft [U](l: Ui.Event.Swipe => U) : Gen.Event.Control = onSwipe(l, real.onSwipeLeftProperty)
  def onSwipeRight[U](l: Ui.Event.Swipe => U) : Gen.Event.Control = onSwipe(l, real.onSwipeRightProperty)
  def onSwipeUp   [U](l: Ui.Event.Swipe => U) : Gen.Event.Control = onSwipe(l, real.onSwipeUpProperty)

  // ------------------------------------------------------------------------------------------------
  import javafx.scene.input.{ SwipeEvent => EVENT }

  private def onSwipe[U](l: Ui.Event.Swipe => U, op: ObjectProperty[EventHandler[_ >: EVENT]]) =
    _onFxEvent[EVENT, Ui.Event.Swipe, U](
        op.cast[ObjectProperty[EventHandler[EVENT]]],
        new Ui.Event.Swipe(_, this),
        l
    )
