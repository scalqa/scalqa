...
transparent trait _onSwipe:
  ...
  def onSwipeDown [U](l: Event.Swipe => U) : Event.Control = onSwipe(l, real.onSwipeDownProperty)
  def onSwipeLeft [U](l: Event.Swipe => U) : Event.Control = onSwipe(l, real.onSwipeLeftProperty)
  def onSwipeRight[U](l: Event.Swipe => U) : Event.Control = onSwipe(l, real.onSwipeRightProperty)
  def onSwipeUp   [U](l: Event.Swipe => U) : Event.Control = onSwipe(l, real.onSwipeUpProperty)

  // ------------------------------------------------------------------------------------------------
  import javafx.scene.input.{ SwipeEvent => EVENT }

  private def onSwipe[U](l: Event.Swipe => U, op: ObjectProperty[EventHandler[_ >: EVENT]]) =
    _onFxEvent[EVENT, Event.Swipe, U](
        op.cast[ObjectProperty[EventHandler[EVENT]]],
        new Event.Swipe(_, this),
        l
    )
