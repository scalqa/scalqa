package scalqa; package fx; package `abstract`; package delegate; import gui.*; import language.implicitConversions
...
trait Gui extends Event.Store.Provider:
  ...
  protected     def _onRealCreated[U](f: REAL => U)                   : Event.Control     = eventStore.onEvent1(Gui.RealCreatedEvent, f).limitRunsTo(1)
  protected[fx] def _onFxEvent[T<:E,U]  (p:OP[EH[T]], l: () => U)     : Event.Control     = _onFxEvent[T,U](p, Event.Id.map1[T,U](l, _ => l()))
  protected[fx] def _onFxEvent[T<:E,M,U](p:OP[EH[T]], as:T=>M,l: M=>U): Event.Control     = _onFxEvent[T,U](p, Event.Id.map1[T,U](l, new Z.MappedEvent(as,l)))
  protected[fx] def _onFxEvent[T<:E,U](  p:OP[EH[T]], l: T => U)      : Event.Control     = {val s=eventStore; if(p.get==null) p.set(new Z.EventDispatch[T,U](s,p)); s.setupEvent(p,l)}
