...
class Group:
  ...
  val items             : Idx.M[Toggle] = Idx.M.wrap(real.getToggles).mutableMapView[Toggle]
  def add(v: Toggle)    : Unit          = items add v
  def select(v: Toggle) : Unit          = real.selectToggle(v.real)
  def selectedPro       : Pro.O[Toggle] = Fx.JavaFx.To.pro_O(real.selectedToggleProperty).mapView(Toggle.FxConverter)
  def selected          : Toggle        = Toggle.FxConverter(real.getSelectedToggle)

object Group:
  def apply()           : Group = new Group()
  def apply(b: Button*) : Group = apply().self(_.items ++= b)
