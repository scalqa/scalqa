...
class Menu extends Control:
  ...
  val items                                           : Idx.M[fx.Menu.Item] = Idx.M.wrap(real.getItems).mutableMapView[fx.Menu.Item]
...
object Menu:
  ...
  def apply(p: Stream[Action]): Menu = Menu().self(_.items ++= p.map(Fx.Menu.Item.apply))
