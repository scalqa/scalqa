package scalqa; package fx; package control; import action.*; import language.implicitConversions
...
class Action(label: String = \/) extends Abstract.Delegate.Gui:
  ...
  /**/               def onAction[U](l: Event => U) : Fx.Event.Control = _onFxEvent[ActionEvent,U](real.onActionProperty, Gen.Event.Id.map1(l,(e: ActionEvent) => l(Event(e))))
  /**/               def onActionRun[U](run: => U)  : Fx.Event.Control = onAction(e => run)
  ...
  @tn("enable_Pro")  def enable_*                   : Pro.OM[Boolean]  = Fx.JavaFx.As.pro_OM(real.disableProperty).twoWay_^(using zBooleanReverse)
...
object Action:
  ...
  def apply(n: Node)                                        : Action = n match { case v: Action => v; case v => new Menu.Item.Custom(v) }
  def apply(s: SEPARATOR)                                   : Action = new z.Separator
  def apply(n: Node.Like)                                   : Action = new z.Basic(n)
  def apply(n1: Node.Like, n2: Node.Like)                   : Action = Action(Pane(n1, n2))
  ...
  implicit inline def xxNode(v: Node)                       : Action = apply(v)
  implicit inline def xxRequest(inline v: SEPARATOR)        : Action = apply(v)

  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  type Event    = action.Event; inline def Event = action.Event
  type Provider = action.Provider

private object zBooleanReverse extends TwoWayFun[Boolean,Boolean]:
