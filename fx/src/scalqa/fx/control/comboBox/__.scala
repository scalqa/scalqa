...
object ComboBox:
  ...
  def apply[A](v: ~[A])                   (using t: Any.Def.Doc[A]) : ComboBox[A] = apply[A]().^(_.items ++= v)
  def apply[A](v: ~[A], selected: Opt[A]) (using t: Any.Def.Doc[A]) : ComboBox[A] = apply[A](v).^(v => selected.forval(v.selection.select(_)))
