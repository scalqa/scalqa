...
class Button extends control.Button.Base:
  private def this(text: String, n: Node.Like) = { this(); _createRealOverride(new REAL(text, n.real)) }
  protected type REAL = javafx.scene.control.MenuButton
  protected override def _createReal = new REAL

  def items: Idx.M[Item] = Idx.M.javaList_^(real.getItems).twoWay_^[Item]

object Button:
  def apply()                                        : Button = new Button
  def apply(text: String, n: Node.Like)              : Button = new Button(text, n)
  def apply(text: String, n: Node.Like, it: ~[Item]) : Button = apply(text, n).^(_.items ++= it)
