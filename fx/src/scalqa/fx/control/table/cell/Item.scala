...
class Item[ROW,V,A](val row: ROW, val setup: Cell.Setup[ROW,V,A], @tn("valueOpt") val value_? : Opt[A]) extends Able.Doc:
  ...
  @tn("textOpt") def text_?           : Opt[String]   = value_?.map(v => if (!setup.voidDef.value_isVoid(v)) setup.funFormat(v) else setup.funFormatVoid(row)) or setup.funFormatVoid(row)
