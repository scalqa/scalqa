package scalqa; package fx; package scene; package chart; package xy; package as; import language.implicitConversions
...
class Basic[X, Y, XA <: Chart.Axis[X], YA <: Chart.Axis[Y]](override val axisX: XA, override val axisY: YA) extends Chart.XY[X, Y](axisX, axisY):
  protected type REAL = z.Basic.Real[X, Y, XA, YA, axisX.VALUE, axisY.VALUE]; override def _createReal = new REAL(this, axisX.real, axisY.real)
  type ITEM <: Item
  type SERIES <: Series

  def item_~ : ~[ITEM] = data.~.flatMap(_.items).dropVoid

  protected[as] object Setup:
  ...
  class Item(xPar: X, yPar: Y) extends ItemBase:
    ...
    def isVisible = true
    protected[as] var layoutJob: () => Unit = \/
