...
object calcScope:
  ...
  def apply[A](axis: As.Custom[A], r: <>[Double], scale: Double, labelSize: Double): Scope[Double] =
    val xr = if (axis.setup.forceZeroInRange && !r.contains(0)) r join 0 else r
    val range = xr.end - xr.start; if (range <= 0) return new Scope(0, 100, 10, "0")
    ...
    new Scope(min, max, unit, format) //(r => ("Result "+ r).TP)
