...
object Key:
  ...
  def subscribe[U](node: Abstract.Delegate.Gui, l: Event.Key => U, op: ObjectProperty[EventHandler[_ >: KeyEvent]]) =
    node._onFxEvent[KeyEvent, Event.Key, U](
      op.cast[ObjectProperty[EventHandler[KeyEvent]]],
      new Event.Key(_, node),
      l
    )
