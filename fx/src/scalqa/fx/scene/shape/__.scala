...
abstract class Shape extends Node:
  ...
  @tn("fill_Pro")   def fill_*            : Pro.OM[Paint] = Fx.JavaFx.As.pro_OM(real.fillProperty)  .twoWay_^[Paint]
  /**/              def fill              : Paint         = fill_*()
  /**/              def fill_=(p: Paint)  : Unit = fill_*() = p

  @tn("stroke_Pro") def stroke_*          : Pro.OM[Paint] = Fx.JavaFx.As.pro_OM(real.strokeProperty).twoWay_^[Paint]
