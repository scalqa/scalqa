...
class Tab extends Control:
  ...
  lazy val tabs            : Idx.M[Tab.Panel]        = Idx.M.wrap(real.getTabs).mutableMapView[Tab.Panel]
  lazy val selection       : Fx.Selection[Tab.Panel] = Fx.Selection(Idx.wrap(real.getTabs), real.getSelectionModel).mapView(Tab.Panel.FxConverter.apply)
  /**/ def sidePro         : Pro.OM[Side]            = Fx.JavaFx.To.pro_OM(real.sideProperty).mutableMapView[Side]
...
object Tab:
  ...
  def apply(s: Side) : Pane.Tab = apply().self(_.side = s)
