...
object HPos extends Fx.JavaFx.Enum.Companion[HPos, JHPos]:
  implicit inline def implicitRequest(inline v: CENTER) : HPos = Center
  implicit inline def implicitRequest(inline v: LEFT)   : HPos = Left
  implicit inline def implicitRequest(inline v: RIGHT)  : HPos = Right
