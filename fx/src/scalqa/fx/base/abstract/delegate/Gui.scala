...
trait Gui extends Gen.Event.Store.Provider:
  ...
  private        var realOpt                                           : Opt[REAL]         =VOID
  protected      def _createReal                                       : REAL
  protected      def _createRealOverride(real: => REAL)                : Unit               = eventStore.setPropertyOpt(Gui.CreateRealOverrideEvent, () => real)
  ...
  private  [fx]  def _refPropertyOpt                                   : Opt[OP[_ <:EH[_]]]=VOID
  /**/           def sceneOpt                                          : Opt[Scene]
  override       def toString                                          : String             = this.self.id + (realOpt.map(_.toString.takeFrom("[", "")) or "")

  final def real: REAL = realOpt or synchronized { // cannot use lazy val, it fails on recursive calls
    realOpt or {
      try {
        val r = eventStore.removePropertyOpt[() => REAL](Gui.CreateRealOverrideEvent).map(_()) or _createReal
        realOpt = r
        ...
        case t: Throwable => { println("Exception in Fx setup for " + this.self.id); throw t }
      }
    }
  }
...
object Gui:
  ...
  def apply[A <: Delegate.Gui](p: OP[_ <: EH[_]]): A = p.get.cast[Z.RefEventDispatch].gui.cast[A]
