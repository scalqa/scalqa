...
object Key:
  ...
  def subscribe[U](node: Abstract.Delegate.Gui, l: Fx.Event.Key => U, op: ObjectProperty[EventHandler[_ >: KeyEvent]]) =
    node._onFxEvent[KeyEvent, Fx.Event.Key, U](
      op.cast[ObjectProperty[EventHandler[KeyEvent]]],
      new Fx.Event.Key(_, node),
      l
    )
