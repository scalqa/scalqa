...
import javafx.geometry.{ Insets => REAL }

object Insets extends Abstract.Delegate.Opaque[Fx.Insets, REAL]("Fx.Insets"):
  inline   def apply(inline top: Double, inline right: Double, inline bottom: Double, inline left: Double) : Insets = new REAL(top, right, bottom, left).cast[Insets]
  inline   def apply(all: Double)                                                                          : Insets = apply(all, all, all, all)
  override def value_doc(v: Insets) = Doc(typeName) += ("top", v.top) += ("bottom", v.bottom) += ("left", v.left) += ("right", v.right)
