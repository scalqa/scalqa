...
sealed class Selection[A](val target: Idx[A], val real: SelectionModel[_ <: Any]) extends Idx.Selection.Observable[A]:
  ...
  /**/     lazy  val indexes                    : Idx[Int]          = real match { case v: MSM[_] => Idx.wrap(v.getSelectedIndices).map_^(_.intValue); case v => zIndexes}
  ...
  private class zEvent[U](f: Selection[A] => U) extends Gen.Event.Control.X.Basic(f) with javafx.beans.InvalidationListener:
