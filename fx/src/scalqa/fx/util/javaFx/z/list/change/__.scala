package scalqa; package fx; package util; package javaFx; package z; package list; package change; import language.implicitConversions

import javafx.{ collections as JFX }

object Change:

  def apply[A](in: Idx[Idx.O.Event[A]], o: JFX.ObservableList[A]): JFX.ListChangeListener.Change[A] & Able.Info =
    ...
    new JFX.ListChangeListener.Change[A](o) with Able.Info {
      private var i = -1
      ...
      def info = new Info(this){ override def text = frames.~.toText }
    }
