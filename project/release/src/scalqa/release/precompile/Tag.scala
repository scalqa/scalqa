...
class Tag(val id: String, val index: Int, t: String) extends Able.Doc:
  ...
  def insertInto(program: String): String =
    var txt = program
    def findPos(start:Int): Int = {var i=txt.indexOf(id,start); while(i>0 && txt.charAtOpt(i+id.length).drop(c => Tag.endChars.contains(c))) i=findPos(i+id.length); i}
    var pos = findPos(0)
    ...
    val codeStart : Int    = txt.charIndexOpt(!_.isSpaceChar, lineStart).get
    val startSpace: String = txt.takeRange(lineStart <>> codeStart)
    if(txt.takeRange(lineStart <>> pos).contains(";"))
      lineStart = txt.lastIndexOf(";",pos)+2
      txt = txt.takeFirst(lineStart-1)+"\n"+startSpace+txt.dropFirst(lineStart-1).trimStart
    else if(txt.takeRange(codeStart <>= 4 ) == "/**/")
      txt = txt.dropRange(codeStart <>> txt.charIndexOpt(!_.isSpaceChar, codeStart+4).get)
    txt = txt.insertAt(lineStart, startSpace + "/**" + lineStream.map(v => "\n" + startSpace + "* " + v).makeString(" ") + "\n" + startSpace + "*/\n")
    txt

  def lineStream: Stream[String] =
    val off = text.lineStream.dropFirst(1).drop(_.trim.length==0).map(v => v.length - v.trimStart.length).minOpt or 0
    var b = true
    text.lineStream.map(v => if(b){b=false; v.trim} else if(v.length > off) v.dropFirst(off) else v)
...
object Tag:
  val Ids = Pack("object", "trait", "class", "def", "val", "var", "type")

  private val endChars : Char.Pack = Char.Pack(' ', '(', '[', ':', '=', '\t')
