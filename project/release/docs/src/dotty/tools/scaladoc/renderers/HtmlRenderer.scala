  ...
  def render(): Unit = // scalqa start ----------------------------------------------------
    ...
    def prepare(page: Page, parents: Vector[Link]): Seq[(String,()=>Unit)] =
      ...
      def run: Unit = page.content match
        ...
        case _ =>
          val pg = page.self.mapIf(_.link.dri.isTypeDef, p => { val d = p.link.dri; Page(Link(d.scalqaLabel().takeAfterLast("."),d),Registry.memberOpt(d.id) or {"\n@@@ "+d.id +"   " + d.tag tp(); throw new RuntimeException()} , Nil) })
    ...
    allPages.flatMap(prepare(_, Vector.empty)).stream.parallel.foreach(_._2())
    // scalqa end --------------------------------------------------------------------------
