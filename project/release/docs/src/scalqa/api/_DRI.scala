...
trait _DRI:
  ...
  extension (x: DRI)
    ...
    def tag               : String      = x.location.replace("$$OPAQUE$","").replace("$$",".").^.to(l => x.anchor.^.?.map(_.takeBefore("-")).map(a => l.^.reviseIf(_.endsWith("$"),_.dropLast(1)) + "." + a) or l)
    ...
    def label(base:String): String      =
      ...
      if(x.anchor.isEmpty) return v.^.to(v => v.lastIndexOf_?("._").map(i => v.dropFirst(i + 1)) or v).simpleName
