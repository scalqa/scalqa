...
object Build {
  ...
  lazy val docs    : Project = project.in(file("generate/docs")).dependsOn(core).settings(commonSettings,fork in run := true).settings(cleanMerge:= _cleanMerge.value, merge:= _merge.value)
