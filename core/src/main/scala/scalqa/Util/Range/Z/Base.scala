...
private[Range] trait Base[A] extends Any { self: Range[A] =>
  ...
  private[Range] def adapt(r: Range[A]): this_type = if (thisClass.isAssignableFrom(r.getClass)) r.asInstanceOf[this_type] else make(r.start, r.startExclusive, r.end, r.endExclusive)
}

private[Range] object Base {

  private[Range] def startContains[A](r: Base[A], v: A, x: Boolean = false) = r.startContains(v, x)
  private[Range] def endContains[A](r: Base[A], v: A, x: Boolean = false) = r.endContains(v, x)
  private[Range] def make[A](r: Base[A], start: A, startIn: Boolean, end: A, endIn: Boolean): r.this_type = r.make(start, startIn, end, endIn)
