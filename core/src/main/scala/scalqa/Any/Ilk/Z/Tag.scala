...
private[Ilk] object Tag {
  ...
  trait ObjectLike[A] extends Tag[A] {
    ...
    @inline def mkMapRefIlk[A, B](s: Stream[A], f: Stream.Mapping[A, B]): Stream[B] = Stream.Z.extend.map.map.refRef(s, f, Refs)
    ...
    @inline def mkLetMapRefIlk[A, B](s: Stream[A], f: Stream.Mapping[A, Opt[B]]) = Stream.Z.extend.filter.letMap.refRef(s, f, Refs)
    @inline def mkLetMapIlkRef[A, B](s: Stream[A], f: Stream.Mapping[A, Opt[B]]) = Stream.Z.extend.filter.letMap.refRef(s, f, Refs)
