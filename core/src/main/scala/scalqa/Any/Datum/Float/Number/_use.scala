package scalqa; package Any; package Datum.Float; package Number

trait _use[TYPE <: _use[TYPE]] extends Any with Computable[TYPE] with Any.Datum.Z.Number.Use[TYPE] {

  def signum: Int = real.signum
  def round(scale: Int, r: Custom.Rounding = \/) = make(r.scale(real, scale).toFloat)
  ...
  def %@(v: Float) = Percent.make(real / v * 100)
