...
private[Long] object stream {
  ...
  abstract class Base extends Stream[Long] with Stream.A.Specialized[Long] {
    ...
    def foreach(c: Stream.Consumer[Long]) = while (prime) c.accept(pump)

    override def ilkOpt = Ilk.Longs
  ...
  class Up(var cur: Long) extends Base {
    ...
    override def sortedOpt = Ordering.Long
  }
  ...
  class Down(var cur: Long) extends Base {
    ...
    override def sortedOpt = Ordering.Long.reverse
  }
}
