package scalqa; package `def`; package float; package g; import language.implicitConversions; import G.Opt
...
object Opt extends z.opt._base:
  ...
  implicit       inline def xx_Boolean[A<:RAW](inline v: Opt[A]): Boolean = java.lang.Float.isNaN(v.real).not

  extension[A<:RAW](inline x: Opt[A])
    /**/               inline def real                                       : Float      = x.cast[Float]
    @tn("is_Void")     inline def isEmpty                                     : Boolean    = java.lang.Float.isNaN(x.real)
    @tn("not_Void")    inline def nonEmpty                                   : Boolean    = java.lang.Float.isNaN(x.real).not
    /**/               inline def raw                                         : Opt[A]     = x
  extension[A<:RAW](x: Opt[A])
    /**/               inline def ref                                         : Val.Opt[A] = (if(x.isEmpty) ZZ.None else java.lang.Float.valueOf(x.real)).cast[Val.Opt[A]]
  ...
  object opaque { opaque type `type`[+A<:RAW] <: Opaque.Float = Float & Opaque.Float }
