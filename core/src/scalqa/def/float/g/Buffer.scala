...
class Buffer[A<:RAW](_a: Array[Float], _sz: Int) extends Val.Buffer[A] with Idx.Mutable[A]:
  ...
  /**/                  def size                            : Int            = sz
  @tn("stream")override def ~                               : Stream[A]      = `def`.array.z.stream.Floats(ar,sz)
  ...
  /**/         override def contains(v: A)                  : Boolean        = `def`.array.z.contains.float(ar,v,sz)
...
private[scalqa] object Buffer:
  ...
  def accessible[A<:RAW](initSize: Int.Opt = \/)          : Buffer[A] & Able.Access[Array[Float]] = zAccessible(new Array[Float](initSize or J.initSize),0)
