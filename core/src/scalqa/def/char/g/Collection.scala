...
trait Collection[A<:RAW] extends Val.Collection[A] with Able.Contain[A] with any.self.info.Specialized.OnChar:
...
object Collection:
  ...
  class StableSet[A<:RAW] private(real: IntMap[Unit]) extends Val.StableSet[A] with Collection[A]:
    ...
    @tn("stream") def ~                  : Stream[A]    = real.keysIterator.~.raw.map(_.Char.cast[A])
