package scalqa; package `def`; package int; package z; import language.implicitConversions
...
object Range:
  ...
  // **************************************************************************************************************************
  class Stream[A<:RAW](s: Int, eX: Long, step: Long) extends g.Stream[A] with Able.Size with Able.Info:
    ...
    /**/                 def info      : Info         = this.defaultDoc += (i<eX) ? ("from",i.tag) += step.?.drop(_ == 1).map(i=>("step",i.tag))
