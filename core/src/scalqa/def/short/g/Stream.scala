...
abstract class Stream[A<:RAW] extends Val.~[A] with ~~.Custom.Discharge[A] with any.self.info.Specialized.OnShort:
  self =>
...
object Stream:
  ...
  implicit         inline def xxRange  [A<:RAW](inline v: g.Range[A])     : Stream[A]     = v.~
