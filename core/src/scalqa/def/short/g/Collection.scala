...
trait Collection[A<:RAW] extends Val.Collection[A] with Able.Contain[A] with any.self.info.Specialized.OnShort:
...
object Collection:
  ...
  class StableSet[A<:RAW] private(real: IntMap[Unit]) extends Val.StableSet[A] with Collection[A]:
    ...
    @tn("stream") def ~                  : Stream[A]    = real.keysIterator.~.raw.map(_.Short.cast[A])
  ...
  object StableSet:
    ...
    @tn("getVoid")  def void[A<:RAW]                   : StableSet[A] = zVoid.cast[StableSet[A]]; private[g] object zVoid extends StableSet(IntMap.empty) with Void
    implicit inline def xxRequest[A<:RAW](inline v: \/): StableSet[A] = void
