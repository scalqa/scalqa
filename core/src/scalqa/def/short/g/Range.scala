...
class Range[A<:RAW](s: A, sz: Int) extends Val.<>[A] with Able.Size with Able.~[A] with any.self.info.Specialized.OnShort:
...
object Range:
  extension[A<:RAW,T,STM<: Shape.OfStream.Any[T]](inline x: Range[A])
    ...
    /**/                                  inline def flatMap[B>:T](inline f:A=> ~[T])(using inline s: Shape.OfStream.Tag[B,STM]): STM       = g.Stream.flatMap[A,T,STM](x.~)[B](f)(using s)
