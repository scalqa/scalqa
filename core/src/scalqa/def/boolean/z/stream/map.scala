...
object map:
  ...
  inline def apply[A<:RAW,B,STM<:Any[B]](inline x: G.~[A], inline f: A=>B, inline s:Tag[B,STM] ): STM = inline s match
  ...
  inline def APPLY[A<:RAW,B,STM<:Any[B]](inline x: G.~[A], inline f: A=>B, inline s:Tag[B,STM]): STM = inline s match
    case _ : Tag[B,Boolean.G.~[B & Raw.Boolean]] => {class Booleans(x: G.~[A]) extends Map.ToBoolean[B & Raw.Boolean](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Boolean])}; Booleans(x)}
    case _ : Tag[B,Byte   .G.~[B & Raw.Byte   ]] => {class Bytes   (x: G.~[A]) extends Map.ToByte   [B & Raw.Byte   ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Byte   ])}; Bytes(x)   }
    case _ : Tag[B,Char   .G.~[B & Raw.Char   ]] => {class Chars   (x: G.~[A]) extends Map.ToChar   [B & Raw.Char   ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Char   ])}; Chars(x)   }
    case _ : Tag[B,Short  .G.~[B & Raw.Short  ]] => {class Shorts  (x: G.~[A]) extends Map.ToShort  [B & Raw.Short  ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Short  ])}; Shorts(x)  }
    case _ : Tag[B,Int    .G.~[B & Raw.Int    ]] => {class Ints    (x: G.~[A]) extends Map.ToInt    [B & Raw.Int    ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Int    ])}; Ints(x)    }
    case _ : Tag[B,Long   .G.~[B & Raw.Long   ]] => {class Longs   (x: G.~[A]) extends Map.ToLong   [B & Raw.Long   ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Long   ])}; Longs(x)   }
    case _ : Tag[B,Float  .G.~[B & Raw.Float  ]] => {class Floats  (x: G.~[A]) extends Map.ToFloat  [B & Raw.Float  ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Float  ])}; Floats(x)  }
    case _ : Tag[B,Double .G.~[B & Raw.Double ]] => {class Doubles (x: G.~[A]) extends Map.ToDouble [B & Raw.Double ](x){ @tn("readRaw_Opt") def readRaw_? =x.readRaw_?.map(f(_).cast[B & Raw.Double ])}; Doubles(x) }
    case _ : Tag[B,       Val.~[B]]               => {class Refs    (x: G.~[A]) extends Map.ToRef    [B              ](x){ @tn("read_Opt")    def read_?    =x.readRaw_?.map(f(_))};                      Refs(x)    }
