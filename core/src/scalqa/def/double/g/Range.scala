...
class Range[A<:RAW](s: A, e: A, eIn: Boolean) extends Val.<>[A] with Able.Contain[A] with any.self.info.Specialized.OnDouble:
  type THIS_TYPE = Range[A]
  final             def start                           : A             = s
  final             def end                             : A             = e
  final             def endIsIn                         : Boolean       = eIn
  final             def ordering                        : G.Ordering[A] = Double.ordering.cast[G.Ordering[A]]
