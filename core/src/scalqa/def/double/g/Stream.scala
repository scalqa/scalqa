package scalqa; package `def`; package double; package g; import language.implicitConversions

abstract class Stream[A<:RAW] extends Val.~[A] with ~~.Custom.Discharge[A] with any.self.info.Specialized.OnDouble:
  self =>
...
object Stream:
  ...
  extension[A<:RAW,T,STM<:Any[T]](inline x: g.Stream[A])
    /**/           inline def map      [B>:T](inline f: A => B)                  (using inline t:Tag[B,        STM]): STM  = z.stream.map(x,f,t)
    /**/           inline def MAP      [B>:T](inline f: A => B)                  (using inline t:Tag[B,        STM]): STM  = z.stream.map.APPLY(x,f,t)
    /**/           inline def flatMap  [B>:T](inline f: A => ~[B])               (using inline t:Tag[B,        STM]): STM  = z.stream.flatMap(x,f,t)
    /**/           inline def FLAT_MAP [B>:T](inline f: A => ~[B])               (using inline t:Tag[B,        STM]): STM  = z.stream.flatMap.APPLY(x,f,t)
    @tn("map_Opt") inline def map_?    [OPT<:Shape.OfOpt.Any[T]](inline f:A=>OPT)(using inline t:Tag.Opt[T,OPT,STM]): STM  = z.stream.mapOpt(x,f,t)
    @tn("MAP_Opt") inline def MAP_?    [OPT<:Shape.OfOpt.Any[T]](inline f:A=>OPT)(using inline t:Tag.Opt[T,OPT,STM]): STM  = z.stream.mapOpt.APPLY(x,f,t)
  // -------------------------------------------------------------------------------------------------------------------------------------------------------
  /**/             inline def apply    [A<:RAW](inline v:A)               : Stream[A]     = Z.Stream_ofOne(v)
  /**/                    def apply    [A<:RAW](v: A*)                    : Stream[A]     = v match{ case v: scala.collection.immutable.ArraySeq.ofDouble => v.unsafeArray.~.cast[Stream[A]]; case v => v.~.raw}
