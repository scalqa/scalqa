...
class Pack[A<:RAW] private(_a: Array[SELF], sz: Int) extends ><[A] with Idx[A]:
  ...
  /**/          override def contains(v: A)           : Boolean     = `def`.array.z.contains.double(ar,v,sz)
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]     = new Pack(`def`.array.z.joinAllAt.double(ar,i,vs.cast[~[SELF]],sz))
