package scalqa; package `def`; package double; package g; import language.implicitConversions; import G.Opt
...
object Opt extends z.opt._base:
  ...
  implicit       inline def xx_Boolean[A<:RAW](inline v: Opt[A]): Boolean = java.lang.Double.isNaN(v.real).not

  extension[A<:RAW](inline x: Opt[A])
    /**/               inline def real                                       : Double     = x.cast[Double]
    @tn("is_Void")     inline def isEmpty                                     : Boolean    = java.lang.Double.isNaN(x.real)
    @tn("not_Void")    inline def nonEmpty                                   : Boolean    = java.lang.Double.isNaN(x.real).not
    /**/               inline def raw                                         : Opt[A]     = x
  extension[A<:RAW](x: Opt[A])
    /**/               inline def ref                                         : Val.Opt[A] = (if(x.isEmpty) ZZ.None else java.lang.Double.valueOf(x.real)).cast[Val.Opt[A]]
    ...
    /**/               inline def dropVoid(using inline t: Info.Tag.Void[A])  : Opt[A]     = if(x.nonEmpty && t.isVoid(x.`val`)) \/ else x
  ...
  extension[A<:RAW,T,OPT<:Any[T]](inline x:Opt[A])
    /**/               inline def map    [B>:T](inline f: A => B)                    (using inline s: Tag[B,OPT]) : OPT    = z.opt.map(x,f,s)
  extension[A<:RAW,T](inline x:Opt[A])
    @tn("map_Opt")     inline def map_?  [OPT<:Any[T]](inline f: A=>OPT)             (using inline s: Tag[T,OPT]) : OPT    = z.opt.mapOpt(x,f,s)
    /**/               inline def flatMap[OPT<:Any[T]](inline f: A=>OPT)             (using inline s: Tag[T,OPT]) : OPT    = z.opt.mapOpt(x,f,s)
  extension[A<:RAW,B,C](inline x:Opt[A])
    /**/               inline def mix[OPT<:Any[C]](inline o:Any[B],inline f:(A,B)=>C)(using inline s: Tag[C,OPT]) : OPT    = z.opt.mixOpt(x,o,f,s)

  object opaque { opaque type `type`[+A<:RAW] <: Opaque.Double = Double & Opaque.Double }
