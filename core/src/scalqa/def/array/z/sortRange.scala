...
object sortRange:
  ...
  inline def apply[A,ARRAY<:Any[A]](inline x: ARRAY, inline p: Int, inline sz: Int, inline c:Ordering[A], inline t: Tag[A,ARRAY]): Unit =
    inline t match
              case _ : Tag[A&Raw.Boolean,Array[A&Raw.Boolean]] => boolean(x.cast[Array[Boolean]],p,sz,c.cast[Ordering[Boolean]])
              case _ : Tag[A&Raw.Byte,   Array[A&Raw.Byte   ]] => byte   (x.cast[Array[Byte   ]],p,sz,c.cast[Ordering[Byte   ]])
              case _ : Tag[A&Raw.Char,   Array[A&Raw.Char   ]] => char   (x.cast[Array[Char   ]],p,sz,c.cast[Ordering[Char   ]])
              case _ : Tag[A&Raw.Short,  Array[A&Raw.Short  ]] => short  (x.cast[Array[Short  ]],p,sz,c.cast[Ordering[Short  ]])
              case _ : Tag[A&Raw.Int,    Array[A&Raw.Int    ]] => int    (x.cast[Array[Int    ]],p,sz,c.cast[Ordering[Int    ]])
              case _ : Tag[A&Raw.Long,   Array[A&Raw.Long   ]] => long   (x.cast[Array[Long   ]],p,sz,c.cast[Ordering[Long   ]])
              case _ : Tag[A&Raw.Float,  Array[A&Raw.Float  ]] => float  (x.cast[Array[Float  ]],p,sz,c.cast[Ordering[Float  ]])
              case _ : Tag[A&Raw.Double, Array[A&Raw.Double ]] => double (x.cast[Array[Double ]],p,sz,c.cast[Ordering[Double ]])
              case _ : Tag[A&Ref,        Array[A&Ref        ]] => ref    (x.cast[Array[Ref    ]],p,sz,c.cast[Ordering[Ref    ]])
