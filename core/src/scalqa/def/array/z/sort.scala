...
object sort:
  ...
  inline def apply[A,ARRAY<:Any[A]](inline x: ARRAY, inline c:Ordering[A], inline t: Tag[A,ARRAY]): Unit =
    inline t match
              case _ : Tag[A&Raw.Boolean,Array[A&Raw.Boolean]] => boolean(x.cast[Array[Boolean]],c.cast[Ordering[Boolean]])
              case _ : Tag[A&Raw.Byte,   Array[A&Raw.Byte   ]] => byte   (x.cast[Array[Byte   ]],c.cast[Ordering[Byte   ]])
              case _ : Tag[A&Raw.Char,   Array[A&Raw.Char   ]] => char   (x.cast[Array[Char   ]],c.cast[Ordering[Char   ]])
              case _ : Tag[A&Raw.Short,  Array[A&Raw.Short  ]] => short  (x.cast[Array[Short  ]],c.cast[Ordering[Short  ]])
              case _ : Tag[A&Raw.Int,    Array[A&Raw.Int    ]] => int    (x.cast[Array[Int    ]],c.cast[Ordering[Int    ]])
              case _ : Tag[A&Raw.Long,   Array[A&Raw.Long   ]] => long   (x.cast[Array[Long   ]],c.cast[Ordering[Long   ]])
              case _ : Tag[A&Raw.Float,  Array[A&Raw.Float  ]] => float  (x.cast[Array[Float  ]],c.cast[Ordering[Float  ]])
              case _ : Tag[A&Raw.Double, Array[A&Raw.Double ]] => double (x.cast[Array[Double ]],c.cast[Ordering[Double ]])
              case _ : Tag[A&Ref,        Array[A&Ref        ]] => ref    (x.cast[Array[Ref    ]],c.cast[Ordering[Ref    ]])
