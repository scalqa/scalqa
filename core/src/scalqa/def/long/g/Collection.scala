...
trait Collection[A<:RAW] extends Val.Collection[A] with Able.Contain[A] with any.self.info.Specialized.OnLong:
...
object Collection:
  ...
  object StableSet:
    ...
    @tn("getVoid")  def void [A<:RAW]                  : StableSet[A] = zVoid.cast[StableSet[A]]; private object zVoid extends StableSet(LongMap.empty) with Void
