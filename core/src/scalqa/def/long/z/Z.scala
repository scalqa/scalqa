...
object  Z:
  ...
  class Stream_fromRange[A<:RAW](start: Long, eX: Long, step: Long) extends G.~[A] with Able.Size.Long with Able.Info:
    ...
    /**/               def info      : Info     = this.defaultDoc += (i<eX) ? ("from",i.toString) += ("step",step)
