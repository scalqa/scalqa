...
private[`val`] final class Ref[A,B] private[stable](private[stable] val real: REAL[A,B]) extends Stable[A,B]:
  ...
  /**/               def joinAll(v: ~[(A, B)]) : Stable[A,B] = new Ref(v.foldAs(real)(_ + _))
