...
class _givens:
  ...
  given givenNameTag[A]                              : Self.NameTag[Result[A]]       = Self.NameTag("Result")
  ...
  given givenDocTag[A](using t: Self.DocTag[A])     : Self.DocTag[Result[A]] with
    def tag(v: Result[A]) : String   = if(v.isValue) "Result("+t.tag(v.cast[A])+")" else "Result(Problem("+v.problem.message+"))"
    def doc(v: Result[A]) : Self.Doc = Self.Doc("Result@"+v.self_^.hash)
