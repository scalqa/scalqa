...
private class JavaList_View[A](protected val real: Idx[A]) extends JavaList_View.Base[A]
...
private[scalqa] object JavaList_View:
  ...
  abstract class Base[A] extends java.util.AbstractList[A] with J.Util.Proxy[Idx[A]]:
    def get(i: Int): A = real(i)
    def size           = real.size

  class M[A](protected val real: Idx.Mutable[A]) extends Base[A] with J.Util.Proxy[Idx.Mutable[A]]:
