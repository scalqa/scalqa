...
transparent trait _drop :
  extension[A](inline x: Stream[A])
    ...
    inline def dropSequence(inline seq: Stream[A])                  : Stream[A] = new  z._build._mutate.replaceSequence(x,  seq.pack,VOID)
    inline def dropSequenceBy[B](inline f:A=>B,inline seq:Stream[B]): Stream[A] = new  z._build._mutate.replaceSequence(x,f,seq.pack,VOID)
