...
import z.build.order

transparent trait _order:

  extension[A](inline x: ~[A])
    inline def sort                                                   (using inline o:Ordering[A]): ~[A] = new order.sort(x)
    inline def sortReversed                                           (using inline o:Ordering[A]): ~[A] = new order.sortReversed(x)
    inline def sortBy[B]    (inline f: A=>B)                          (using inline o:Ordering[B]): ~[A] = new order.sortBy(x, f)
    inline def sortBy[B,C]  (f1:A=>B,f2:A=>C)                    (using  Ordering[B], Ordering[C]): ~[A] = new order.sortBy(x, v => (f1(v), f2(v)))
    inline def sortBy[B,C,D](f1:A=>B,f2:A=>C,f3:A=>D)(using Ordering[B], Ordering[C], Ordering[D]): ~[A] = new order.sortBy(x, v => (f1(v), f2(v), f3(v)))
