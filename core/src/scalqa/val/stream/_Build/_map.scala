...
transparent trait _map:
  ...
  extension[A,T,STM<:Any[T]](inline x: ~[A])
    /**/            inline def map     [B>:T](inline f: A => B)              (using inline s:Tag[B,STM]): STM  = M.map(x,f,s)
    /**/            inline def MAP     [B>:T](inline f: A => B)              (using inline s:Tag[B,STM]): STM  = M.map.APPLY(x,f,s)
    /**/            inline def flatMap [B>:T](inline f: A => ~[B])           (using inline s:Tag[B,STM]): STM  = M.flatMap(x,f,s)
    /**/            inline def FLAT_MAP[B>:T](inline f: A => ~[B])           (using inline s:Tag[B,STM]): STM  = M.flatMap.APPLY(x,f,s)
    /**/            inline def flatten           (using inline f:A=>Able.~[T], inline s:Tag[T,STM]): STM  = M.flatMap(x,f(_).~,s)
    @tn("map_Opt")  inline def map_?   [OPT<:AnyOpt[T]](inline f:A=>OPT) (using inline s:Tag.Opt[T,OPT,STM]): STM  = M.mapOpt(x,f,s)
    @tn("MAP_Opt")  inline def MAP_?   [OPT<:AnyOpt[T]](inline f:A=>OPT) (using inline s:Tag.Opt[T,OPT,STM]): STM  = M.mapOpt.APPLY(x,f,s)
