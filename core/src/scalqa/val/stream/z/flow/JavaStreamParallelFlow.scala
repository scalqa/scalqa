...
class JavaStreamParallelFlow[A] private (val name: String, jStream: java.util.stream.Stream[A], szOpt: Long.Opt, override val base: AnyRef) extends custom.pipeline.ParallelFlow[A] with Custom.Pipeline:
  def this(v: Stream[A]) = this(\/, v.toJavaStream(true), v.sizeLongOpt, v)
  ...
  def flatMap[B](f: A => Stream[B])             : JavaStreamParallelFlow[B] = JavaStreamParallelFlow[B]("flatMap", jStream.flatMap[B](f(_).toJavaStream(true)), \/, this)
