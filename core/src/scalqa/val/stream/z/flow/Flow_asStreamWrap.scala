...
class Flow_asStreamWrap[A](s: ~[A]) extends Flow[A] with Custom.Pipeline:
  private var x: ~[A] = s
  /**/             inline def isParallel                                   : Boolean           = false
  @tn("stream")           def ~                                            : ~[A]              = x
  @tn("sizeLong_Opt")     def sizeLong_?                                   : Long.Opt          = s.sizeLong_?
  protected      override def base                                         : ~[A]              = x
  /**/                    def takeType[B](using t: ClassTag[B])            : Flow[B]           = this.cast[Flow_asStreamWrap[B]].^(_.x = x.takeType[B])
  /**/                    def collect[B](f: PartialFunction[A,B])          : Flow[B]           = this.cast[Flow_asStreamWrap[B]].^(_.x = x.collect(f))
  /**/                    def take(f: A => Boolean)                        : Flow[A]           = this.^(_.x = x.take(f))
  /**/                    def drop(f: A => Boolean)                        : Flow[A]           = this.^(_.x = x.drop(f))
  /**/                    def map[B](f: A => B)                            : Flow[B]           = this.cast[Flow_asStreamWrap[B]].^(_.x = x.map(f))
  @tn("map_Opt")          def map_?[B](f: A => Opt[B])                     : Flow[B]           = this.cast[Flow_asStreamWrap[B]].^(_.x = x.map_?(f))
  /**/                    def flatMap[B](f: A => ~[B])                     : Flow[B]           = this.cast[Flow_asStreamWrap[B]].^(_.x = x.flatMap(f))
