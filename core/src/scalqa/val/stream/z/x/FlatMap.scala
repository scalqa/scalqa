...
abstract class FlatMap[A](x: AnyRef) extends Pipe[A](x):
    ...
    private            def fetchOpt  : Opt[A]              = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readOpt})
...
object FlatMap:
  abstract class AsBoolean(x: AnyRef) extends Pipe.asBoolean(x):
    private            var cur       : Boolean.Stream     =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Boolean.Stream]
    private            def fetchOpt  : Boolean.Opt         = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsByte(x: AnyRef) extends Pipe.asByte(x):
    private            var cur       : Byte.Stream        =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Byte.Stream]
    private            def fetchOpt  : Byte.Opt            = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsChar(x: AnyRef) extends Pipe.asChar(x):
    private            var cur       : Char.Stream        =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Char.Stream]
    private            def fetchOpt  : Char.Opt            = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsShort(x: AnyRef) extends Pipe.asShort(x):
    private            var cur       : Short.Stream       =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Short.Stream]
    private            def fetchOpt  : Short.Opt           = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsInt(x: AnyRef) extends Pipe.asInt(x):
    private            var cur       : Int.Stream         =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Int.Stream]
    private            def fetchOpt  : Int.Opt             = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsLong(x: AnyRef) extends Pipe.asLong(x):
    private            var cur       : Long.Stream        =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Long.Stream]
    private            def fetchOpt  : Long.Opt            = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsFloat(x: AnyRef) extends Pipe.asFloat(x):
    private            var cur       : Float.Stream       =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Float.Stream]
    private            def fetchOpt  : Float.Opt           = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
  ...
  abstract class AsDouble(x: AnyRef) extends Pipe.asDouble(x):
    private            var cur       : Double.Stream      =VOID
    private            var open      : Boolean             = true
    protected          def fewOpt    : Opt[Double.Stream]
    private            def fetchOpt  : Double.Opt          = fewOpt.fornil{cur=VOID; open = false}.mapOpt(v => { cur = v; cur.readRawOpt})
