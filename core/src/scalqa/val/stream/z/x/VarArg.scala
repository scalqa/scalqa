...
object VarArg:
  ...
  class Stream_ofMany[A](v1: A, v2: A, v3: A, vs: Seq[A]) extends Stream[A] with Able.Size:
    ...
    def readOpt : Opt[A]  = { i+=1; if(i>3){ if(i>sz) \/ else vs(i-4)} else if(i==1) v1 else if(i==2) v2 else if(i==3) v3 else \/ }
