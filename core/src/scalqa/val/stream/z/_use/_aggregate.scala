...
object _aggregate:
  ...
  transparent inline def _foldAs[A,B](inline x: ~[A], inline s: B, inline f: (B,A) => B) : B  = inline x match
    case _ : ~[A & Boolean.Raw] => { var r = s; _process.foreachBoolean(x.cast[~[Boolean]],v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Byte.Raw]    => { var r = s; _process.foreachByte(   x.cast[~[Byte]],   v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Short.Raw]   => { var r = s; _process.foreachShort(  x.cast[~[Short]],  v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Char.Raw]    => { var r = s; _process.foreachChar(   x.cast[~[Char]],   v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Int.Raw]     => { var r = s; _process.foreachInt(    x.cast[~[Int]],    v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Long.Raw]    => { var r = s; _process.foreachLong(   x.cast[~[Long]],   v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Float.Raw]   => { var r = s; _process.foreachFloat(  x.cast[~[Float]],  v => r = f(r,v.cast[A])); r}
    case _ : ~[A & Double.Raw]  => { var r = s; _process.foreachDouble( x.cast[~[Double]], v => r = f(r,v.cast[A])); r}
    case _                      => foldAs(x,s,f)
  ...
  inline def _fold[A](inline x: ~[A], inline s: A, inline f: (A,A) => A) : A  = inline s match
    case _ : Boolean.Raw => foldBoolean(x.cast[~[Boolean]],s.cast[Boolean],(x,y) => f(x.cast[A],y.cast[A]).cast[Boolean]).cast[A]
    case _ : Byte.Raw    => foldByte   (x.cast[~[Byte]],   s.cast[Byte],   (x,y) => f(x.cast[A],y.cast[A]).cast[Byte   ]).cast[A]
    case _ : Short.Raw   => foldShort  (x.cast[~[Short]],  s.cast[Short],  (x,y) => f(x.cast[A],y.cast[A]).cast[Short  ]).cast[A]
    case _ : Char.Raw    => foldChar   (x.cast[~[Char]],   s.cast[Char],   (x,y) => f(x.cast[A],y.cast[A]).cast[Char   ]).cast[A]
    case _ : Int.Raw     => foldInt    (x.cast[~[Int]],    s.cast[Int],    (x,y) => f(x.cast[A],y.cast[A]).cast[Int    ]).cast[A]
    case _ : Long.Raw    => foldLong   (x.cast[~[Long]],   s.cast[Long],   (x,y) => f(x.cast[A],y.cast[A]).cast[Long   ]).cast[A]
    case _ : Float.Raw   => foldFloat  (x.cast[~[Float]],  s.cast[Float],  (x,y) => f(x.cast[A],y.cast[A]).cast[Float  ]).cast[A]
    case _ : Double.Raw  => foldDouble (x.cast[~[Double]], s.cast[Double], (x,y) => f(x.cast[A],y.cast[A]).cast[Double ]).cast[A]
    case _               => fold(x,s,f)
  ...
  inline def _reduce_Opt[A](x: ~[A], inline f: (A,A) => A) : Opt[A] = inline x match
    case _ :  ~[A & Boolean.Raw] => reduceBoolean_Opt(x.cast[~[Boolean]],(x,y) => f(x.cast[A],y.cast[A]).cast[Boolean]).cast[A]
    case _ :  ~[A & Byte.Raw]    => reduceByte_Opt   (x.cast[~[Byte]],   (x,y) => f(x.cast[A],y.cast[A]).cast[Byte   ]).cast[A]
    case _ :  ~[A & Short.Raw]   => reduceShort_Opt  (x.cast[~[Short]],  (x,y) => f(x.cast[A],y.cast[A]).cast[Short  ]).cast[A]
    case _ :  ~[A & Char.Raw]    => reduceChar_Opt   (x.cast[~[Char]],   (x,y) => f(x.cast[A],y.cast[A]).cast[Char   ]).cast[A]
    case _ :  ~[A & Int.Raw]     => reduceInt_Opt    (x.cast[~[Int]],    (x,y) => f(x.cast[A],y.cast[A]).cast[Int    ]).cast[A]
    case _ :  ~[A & Long.Raw]    => reduceLong_Opt   (x.cast[~[Long]],   (x,y) => f(x.cast[A],y.cast[A]).cast[Long   ]).cast[A]
    case _ :  ~[A & Float.Raw]   => reduceFloat_Opt  (x.cast[~[Float]],  (x,y) => f(x.cast[A],y.cast[A]).cast[Float  ]).cast[A]
    case _ :  ~[A & Double.Raw]  => reduceDouble_Opt (x.cast[~[Double]], (x,y) => f(x.cast[A],y.cast[A]).cast[Double ]).cast[A]
