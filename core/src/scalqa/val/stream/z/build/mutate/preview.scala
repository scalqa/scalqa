...
class preview[A](src: ~[A]) extends a.Pipe[A](src) with ~~.Preview[A] with Able.Size.Opt:
  ...
  override                  def info                      : Info             = super.info += ("buffered", stack.size)
  // ---------------------------------------------------------------------------------------------------------------------------
  ...
  // **********************************************************************************************************
  object previewSize extends Preview.LazySize with Able.Info:
    def preload(i: Int): Int = self.preload(i)
    def info           : Info = Info(this) += size_?.map(v=>("size",v.tag)) += ("buffered",stack.size)
