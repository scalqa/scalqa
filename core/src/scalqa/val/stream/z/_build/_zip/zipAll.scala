...
class zipAll[A,B](one: Stream[A], two: Stream[B], ao: Opt[A], bo: Opt[B]) extends Stream[(A, B)] with Custom.Pipeline.Tree with Able.Size.Opt.Long:
  ...
  def readOpt = if(done1 && done2) VOID else
    val o1 : Opt[A] = if(done1) VOID else one.readOpt.fornil{done1=true}
    val o2 : Opt[B] = if(done2) VOID else two.readOpt.fornil{done2=true}
    if(done1 && done2) VOID else (o1 orOpt ao or fail("first"), o2 orOpt bo or fail("second"))
