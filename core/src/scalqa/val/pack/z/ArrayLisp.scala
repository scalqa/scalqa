...
private[`val`] class ArrayPack[A](_a: Array[Ref], sz: Int) extends ><[A] with Able.Info:
  ...
  @tn("stream") override def ~                           : ~[A] & Able.Size = `def`.array.z.stream.Refs(ar,sz)
  ...
  /**/                   def joinAllAt(i: Int, v: ~[A])  : ><[A]            = new ArrayPack(`def`.array.z.joinAllAt.ref(ar,i,v.cast[~[Ref]],sz))
  ...
  /**/                   def info                        : Info             = Info(this) += ("size", size) += (ar.length != sz) ? ("","Unpacked") += ar.tag
