...
transparent trait view:
  ...
  extension[A](x: any._Methods.Self[A])
  ...
  extension[A](inline x: any._Methods.Self[A])
  ...
  extension[A](inline x: any._Methods.Self[~[A]]) // ~ has to be treated differently from others
    @tn("nonEmpty_Opt") inline def ?                                                    : Opt[~[A]] = view.nonEmptyStream_Opt(x.cast[~[A]])


object view:

  def nonEmptyStream_Opt[A](s: ~[A]): Opt[~[A]] = s.sizeLong_?.take(_ > 0).map(_ => s) or_? s.enablePreview.^.map(p => p.preview_?.map[~[A]](_ => p))
