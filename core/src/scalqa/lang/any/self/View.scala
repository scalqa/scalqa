...
import Self.View
import `given`. { NameTag, VoidTag, InTag, DocTag, EmptyTag }

object View:
  extension[A](x: View[A])
  ...
  given givenDocTag[A](using d: DocTag[A]): DocTag[View[A]] with
    def tag(v: View[A]) = doc(v).tag
    def doc(v: View[A]) = d.doc(v.real)
...
@def typeName -> Type name
    ...
    Returns type name, which is class name for reference types
...
@def revise -> Change value
    ...
    Note. The value type stays the same
...
@def ? -> As option
    ...
    Returns self value as an option.
