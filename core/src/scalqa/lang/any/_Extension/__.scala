...
transparent trait _extension:
  extension[A](inline x:A)
    ...
    @tn("selfView")  inline def ^                                                                                       : SelfView[A] = x.cast[SelfView[A]]
    @tn("self_View") inline def self_^                                                                                  : SelfView[A] = x.cast[SelfView[A]]


object _extension:
  @fast lazy val SelfView = _Extension.SelfView; type SelfView[+A] = _Extension.SelfView.opaque.`type`[A]
...
@def ^ -> Self view
    ...
    Returns additional [SelfView](_Extension/SelfView.html) library available to this instance
