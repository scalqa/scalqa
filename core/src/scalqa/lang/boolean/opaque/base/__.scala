...
abstract class Base[A<:Opaque](val typeName:String) extends Any.Opaque.Base[A](ClassTag.Boolean.cast[ClassTag[A]]) with Base._methods with gen.`given`.VoidDef.BooleanRaw[A]:
