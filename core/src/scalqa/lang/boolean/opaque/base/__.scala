...
abstract class Base[A<:Opaque](name:String) extends Any.Opaque.Base[A](name,ClassTag.Boolean.cast[ClassTag[A]]) with Base._methods with gen.`given`.VoidDef.BooleanRaw[A]:
