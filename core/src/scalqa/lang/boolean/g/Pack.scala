...
class Pack[A<:RAW] private(_a: Array[SELF], sz: Int) extends ><[A] with Idx[A]:
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]     = new Pack(lang.array.z.joinAllAt.boolean(ar,i,vs.cast[~[SELF]],sz))
...
object Pack:
  ...
  @tn("getVoid")  def void      [A<:RAW]                        : Pack[A] = zVoid.cast[Pack[A]]; private object zVoid extends Pack(SELF.emptyArray) with Self.Void
