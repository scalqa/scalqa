...
class Buffer[A<:Raw](arrayToUse: Array[Boolean], sizeToStartWith: Int) extends Val.Buffer[A] with Idx.Mutable[A]:
  ...
  /**/                  def size                            : Int            = sz
  @tn("stream")override def ~                               : Stream[A]      = lang.array.z.As.BooleanStream(ar,sz)
...
object Buffer:
  ...
  /**/     inline def accessible[A<:Raw](inline initSize: Int = J.initSize) : Buffer[A] & Able.Access[Array[Boolean]] = accessible(new Array[Boolean](initSize),0)
