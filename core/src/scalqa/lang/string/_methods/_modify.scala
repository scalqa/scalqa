...
transparent trait _modify:
  ...
  extension(inline x: String)
    @tn("joinTag")        inline def ++[A](inline v: A)               (using inline d: Given.DocDef[A]): String = x + d.value_tag(v)
    @tn("joinOpt")        inline def +?[A](inline v: Opt[A])          (using inline d: Given.DocDef[A]): String = x.^.reviseIf(_ => v.nonEmpty, _ + v.toString)
