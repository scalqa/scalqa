...
object Builder extends Any.Ref.Custom.Type[Builder,StringBuilder]("String.Builder"):
  ...
  extension[A](inline x: Builder)
    @tn("add")   infix inline def += (inline v: A)(using tg :Given.DocTag[A])          : Builder = x.real.append(tg.tag(v)).asOpaque[Builder]
    @tn("addAt") infix inline def +=@(off: Int, inline v: A)(using tg :Given.DocTag[A]): Builder = x.real.insert(off,tg.tag(v)).asOpaque[Builder]
