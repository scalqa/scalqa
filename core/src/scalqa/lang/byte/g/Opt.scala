...
object Opt:
  ...
  implicit       inline def implicitFromAnyByte[A<:Raw](inline v: A)          : Opt[A]     = v.cast[Opt[A]]
  ...
  extension[A<:Raw](inline x: Opt[A])
    ...
    /**/  inline def ref                                         : Val.Opt[A] = {val X=x; if(X.isEmpty) ZZ.None else java.lang.Byte.valueOf(X.real.toByte)}.cast[Val.Opt[A]]
  ...
  object TYPE:
    opaque type DEF[+A<:Raw] <: Short.Opaque = Short.Opaque
...
___________________________________________________________________________*/
/**
@type DEF -> ### Byte Specialized Generic Option
