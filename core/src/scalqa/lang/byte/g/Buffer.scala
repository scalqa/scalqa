...
class Buffer[A<:RAW](_a: Array[Byte], _sz: Int) extends Val.Buffer[A] with Idx.Mutable[A]:
  ...
  /**/                  def size                            : Int            = sz
  @tn("stream")override def ~                               : Stream[A]      = lang.array.z.stream.Bytes(ar,sz)
...
private[scalqa] object Buffer:
  ...
  def accessible[A<:RAW](initSize: Int.Opt = \/)         : Buffer[A] & Able.Access[Array[Byte]] = zAccessible(new Array[Byte](initSize or J.initSize),0)
