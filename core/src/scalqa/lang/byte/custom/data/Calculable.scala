...
abstract class Calculable[A<:Opaque.Byte](typeName:String) extends Ordered[A](typeName) with Calculable._methods[A]:
...
object Calculable:
  ...
  trait _methods[A<:RAW] extends Ordered._methods[A]:
    extension(inline x: A)
      ...
      @tn("multiply") inline def *(inline v: Byte) : A  = (x.real * v).cast[A]
      @tn("divide")   inline def /(inline v: Byte) : A  = (x.real / v).cast[A]
...
___________________________________________________________________________*/
/**
@class Calculable -> ### Byte Custom Calculable Data Setup
...
@trait _methods -> ### Custom Calculable Data Default Methods
