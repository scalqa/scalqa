...
abstract class Calculable[A<:Opaque](name: String) extends Ordered[A](name) with Calculable._methods:
  ...
  @fast given math: G.Math[A] = Byte.math.cast[G.Math[A]]
...
object Calculable:
  ...
  transparent trait _methods extends Ordered._methods:
    extension[THIS_OPAQUE <: Opaque](inline x: THIS_OPAQUE)
      ...
      @tn("multiply") inline def *(inline v: Byte)       : THIS_OPAQUE  = (x.real * v     ).cast[THIS_OPAQUE]
      @tn("divide")   inline def /(inline v: Byte)       : THIS_OPAQUE  = (x.real / v     ).cast[THIS_OPAQUE]
...
___________________________________________________________________________*/
/**
@class Calculable -> ### Byte Opaque Calculable Data Companion Object Base
  ...
  See [sample type definition](https://github.com/scalqa/samples/blob/master/src/example/opaque/all/CalculableByteData.scala).

@trait _methods -> Default methods for all Byte.Opaque.Data.Calculable defined types
