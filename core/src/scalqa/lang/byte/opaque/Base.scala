...
abstract class Base[A<:Opaque](name:String) extends any.z.OpaqueBase[A](name,ClassTag.Byte.cast[ClassTag[A]]) with Base._methods with any.`def`.Void.Primitive.X.Byte[A]:
  def value_isVoid(v: A): Boolean = false
  def value_tag(v: A)   : String  = default_doc(v).tag
  def value_doc(v: A)   : Doc     = default_doc(v)

  given zzVoidDef: Any.Def.Void.Primitive.X.Byte[A] = this
...
object Base:
  ...
  transparent trait _methods:
    extension[THIS_OPAQUE <: Opaque](inline x: ZZ) // Methods will be available from root library instead
      /**/       inline def real: Byte               = J.unsupportedOperation()
...
___________________________________________________________________________*/
/**
@class Base -> ### Byte Opaque Companion Object Base
  ...
  See [sample type definition](https://github.com/scalqa/samples/blob/master/src/example/opaque/all/ByteOpaque.scala).

@trait _methods -> Default methods for all Byte.Opaque.Base defined types
...
@def real -> Real value
     ...
     Returns Byte value standing behind the opaque type.

     This is a zero cost call.

@def ? ->  To option

    Returns Byte specialized option
