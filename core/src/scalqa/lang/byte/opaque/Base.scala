...
abstract class Base[A<:Opaque](name:String) extends Any.Opaque.Base[A](name,ClassTag.Byte.cast[ClassTag[A]]) with Base._methods with gen.`given`.VoidDef.ByteRaw[A]:
