...
abstract class Stream[A<:Raw] extends Val.Stream[A] with Val.Stream.Custom.Discharge[A] with any.z.PrimitiveTag.Long:
  self =>
...
object Stream:
  ...
  /**/                    def apply    [A<:Raw](v: A*)                       : Stream[A] = v match{ case v: scala.collection.immutable.ArraySeq.ofLong => v.unsafeArray.stream.cast[Stream[A]]; case v => v.stream.raw}
  ...
  implicit         inline def implicitFromRange     [A<:Raw](inline v: g.Range[A])     : Stream[A] = v.stream
...
___________________________________________________________________________*/
/**
@class Stream -> ### Long Specialized Generic Stream
