...
class Pack[A<:Raw] private(_a: Array[Long], sz: Int) extends ><[A] with Idx[A]:
  private def this(a: Array[Long]) = this(a,a.length)
  type THIS_TYPE = Pack[A]
  private                var ar                       : Array[Long] = _a
  /**/                   def size                     : Int           = sz
  ...
  /**/                   def compact                  : Pack[A]       = {if(ar.length>sz) {ar=new Array[Long](sz).^(ar.copyTo(_,0,0,sz))}; this}
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]       = new Pack(lang.array.z.joinAllAt.long(ar,i,vs.cast[~[Long]],sz))
  /**/                   def joinAll(vs: ~[A])        : this.type   = vs.read_?.map(v => Pack.Buf(ar,sz+1,v,vs).mk).or(this).cast[this.type]

object Pack:
  /**/                   def fromVarArg[A<:Raw](v: A, vs: Seq[A])      : Pack[A] = new Pack(Array(v.real,vs.cast[Seq[Long]] *))
  /**/            inline def fromArray [A<:Raw](v: Array[Long])        : Pack[A] = fromArray(v,v.length)
  /**/                   def fromArray [A<:Raw](v: Array[Long],sz: Int): Pack[A] = new Pack(v.copySize(sz),sz)
  /**/                   def fromStream[A<:Raw](v: ~[A])               : Pack[A] = void[A].joinAll(v)
  @tn("getVoid")  inline def void      [A<:Raw]                        : Pack[A] = zVoid.cast[Pack[A]]; object zVoid extends Pack(lang.Long.emptyArray) with Gen.Void
  ...
  private class Buf[A<:Raw] private(a: Array[Long],sz:Int) extends Buffer[A](a,sz):
    def this(a: Array[Long], sz: Int, v: A, vs: ~[A]) = { this(a.copySize(vs.size_?.map(sz + _) or sz + J.initSize).^(_(sz-1)=v), sz); addAll(vs) }
    def mk: Pack[A] =
