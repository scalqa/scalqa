...
class Set[A<:Raw] private(real: LongMap[Unit]) extends Val.Set[A] with Collection[A]:
  ...
  /**/     def stream                   : Stream[A] = real.keysIterator.stream.raw.map(_.cast[A])
  /**/     def join(v: A)               : Set[A]    = new Set(real.updated(v.real,()))
  /**/     def joinAll(v: Val.Stream[A]): Set[A]    = new Set(real.concat(v.map(v => (v.real,())).iterator))

object Set:
  /**/                  def apply[A<:Raw](v: A *)                : Set[A] = new Set(LongMap.from(v.map(v => (v.real,()))))
  /**/                  def fromStream[A<:Raw](v: Val.Stream[A]) : Set[A] = new Set(LongMap.from(v.map(v => (v.real,())).iterator))
  @tn("getVoid") inline def void[A<:Raw]                         : Set[A] = z_Void.cast[Set[A]]
  implicit       inline def implicitRequest[A<:Raw](v: \/)       : Set[A] = void

  // **************************************************
  object z_Void extends Set(LongMap.empty) with Gen.Void
