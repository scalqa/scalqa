...
object Set:
  /**/            def apply[A<:Raw](v: A *)      : Set[A] = Set(v)
  /**/            def apply[A<:Raw](v: ~[A])     : Set[A] = new Set(LongMap.from(v.map(v => (v.real,())).iterator))
  @tn("getVoid")  def void [A<:Raw]              : Set[A] = zVoid.cast[Set[A]]; private object zVoid extends Set(LongMap.empty) with Gen.Void
