...
object Lookup:
  ...
  class Stable[A<:Raw,B] private(real: LongMap[B]) extends Val.Lookup.Stable[A,B] with Lookup[A,B]:
    ...
    override def keyStream                     : Stream[A]         = real.keysIterator.stream.raw.map(_.cast[A])
    override def pairStream                    : Val.Stream[(A,B)] = real.stream.map(v => (v._1.cast[A],v._2))
    override def join(k: A, v: B)              : Stable[A,B]       = new Stable(real.updated(k.real, v))
    override def joinAll(v:Val.Stream[(A, B)]) : Stable[A,B]       = new Stable(real.concat(v.map(v => (v._1.real,v._2)).iterator))
