...
abstract class Calculable[A<:Opaque](name: String) extends Ordered[A](name) with Calculable._methods:
  ...
  @fast given math: G.Math[A] = Long.math.cast[G.Math[A]]
...
object Calculable:
  ...
  transparent trait _methods extends Ordered._methods:
    extension[THIS_OPAQUE <: Opaque](inline x: THIS_OPAQUE)
      ...
      @tn("multiply") inline def *(inline v: Byte|Short|Int|Long): THIS_OPAQUE  = (x.real * v.cast[Long]).cast[THIS_OPAQUE]
      @tn("divide")   inline def /(inline v: Byte|Short|Int|Long): THIS_OPAQUE  = (x.real / v.cast[Long]).cast[THIS_OPAQUE]
...
___________________________________________________________________________*/
/**
@class Calculable -> ### Long Opaque Calculable Data Companion Object Base
  ...
  See [sample type definition](https://github.com/scalqa/samples/blob/master/src/example/opaque/all/CalculableLongData.scala).

@trait _methods -> Default methods for all Long.Opaque.Data.Calculable defined types
