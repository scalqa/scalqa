...
object mapOpt:
  ...
  inline def apply[A<:Raw,B,STM<: ~~.AnyType[B],OPT<:Val.Opt.AnyType[B]](inline x: ~[A], inline f: A => OPT, inline s:SHAPE[B,OPT,STM]): STM =
    ...
    inline s match
      ...
      case _ : SHAPE[B,      Val.Opt[BB],Boolean.G.~[BB]] => new toBoolean(x,v => f(v).cast[Opt[BB]].raw)
      case _ : SHAPE[B,      Val.Opt[BY],Byte   .G.~[BY]] => new toByte   (x,v => f(v).cast[Opt[BY]].raw)
      case _ : SHAPE[B,      Val.Opt[BC],Char   .G.~[BC]] => new toChar   (x,v => f(v).cast[Opt[BC]].raw)
      case _ : SHAPE[B,      Val.Opt[BS],Short  .G.~[BS]] => new toShort  (x,v => f(v).cast[Opt[BS]].raw)
      case _ : SHAPE[B,      Val.Opt[BI],Int    .G.~[BI]] => new toInt    (x,v => f(v).cast[Opt[BI]].raw)
      case _ : SHAPE[B,      Val.Opt[BL],Long   .G.~[BL]] => new toLong   (x,v => f(v).cast[Opt[BL]].raw)
      case _ : SHAPE[B,      Val.Opt[BF],Float  .G.~[BF]] => new toFloat  (x,v => f(v).cast[Opt[BF]].raw)
      case _ : SHAPE[B,      Val.Opt[BD],Double .G.~[BD]] => new toDouble (x,v => f(v).cast[Opt[BD]].raw)
      case _ : SHAPE[B,      Val.Opt[B ],      Val.~[B ]] => new mapOpt   (x,v => f(v).cast[Opt[B ]])
  ...
  inline def APPLY[A<:Raw,B,STM<: ~~.AnyType[B],OPT<:Val.Opt.AnyType[B]](inline x: ~[A], inline f: A => OPT, inline s:SHAPE[B,OPT,STM]): STM =
    ...
    inline s match
      ...
      case _ : SHAPE[B,      Val.Opt[BB],Boolean.G.~[BB]] => {class mapOptToBoolean(x: ~[A]) extends Pipe.ofBoolean[BB](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BB]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToBoolean(x)}
      case _ : SHAPE[B,      Val.Opt[BY],Byte   .G.~[BY]] => {class mapOptToByte   (x: ~[A]) extends Pipe.ofByte   [BY](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BY]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToByte   (x)}
      case _ : SHAPE[B,      Val.Opt[BC],Char   .G.~[BC]] => {class mapOptToChar   (x: ~[A]) extends Pipe.ofChar   [BC](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BC]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToChar   (x)}
      case _ : SHAPE[B,      Val.Opt[BS],Short  .G.~[BS]] => {class mapOptToShort  (x: ~[A]) extends Pipe.ofShort  [BS](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BS]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToShort  (x)}
      case _ : SHAPE[B,      Val.Opt[BI],Int    .G.~[BI]] => {class mapOptToInt    (x: ~[A]) extends Pipe.ofInt    [BI](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BI]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToInt    (x)}
      case _ : SHAPE[B,      Val.Opt[BL],Long   .G.~[BL]] => {class mapOptToLong   (x: ~[A]) extends Pipe.ofLong   [BL](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BL]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToLong   (x)}
      case _ : SHAPE[B,      Val.Opt[BF],Float  .G.~[BF]] => {class mapOptToFloat  (x: ~[A]) extends Pipe.ofFloat  [BF](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BF]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToFloat  (x)}
      case _ : SHAPE[B,      Val.Opt[BD],Double .G.~[BD]] => {class mapOptToDouble (x: ~[A]) extends Pipe.ofDouble [BD](x){@tn("readRaw_Opt") def readRaw_? ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[BD]].raw; if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOptToDouble (x)}
      case _ : SHAPE[B,      Val.Opt[B ],      Val.~[B ]] => {class mapOpt         (x: ~[A]) extends Pipe.ofRef    [B ](x){@tn("read_Opt")    def read_?    ={var o=x.read_?; while(o.nonEmpty){val r= f(o.`val`).cast[  Val.Opt[B]];      if(r.nonEmpty) return r; o=x.read_? }; \/}}; new mapOpt         (x)}
