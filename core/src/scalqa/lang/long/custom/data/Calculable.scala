...
abstract class Calculable[A<:Opaque.Long](typeName:String) extends Ordered[A](typeName) with Calculable._methods[A]:
...
object Calculable:
  ...
  trait _methods[A<:RAW] extends Ordered._methods[A]:
    extension(inline x: A)
      ...
      @tn("multiply") inline def *(inline v: Byte|Short|Int|Long) : A  = (x.real * v.cast[Long]).cast[A]
      @tn("divide")   inline def /(inline v: Byte|Short|Int|Long) : A  = (x.real / v.cast[Long]).cast[A]
