...
abstract class Numerical[A<:Opaque.Long](typeName:String) extends Ordered[A](typeName) with Numerical._methods[A]:
...
object Numerical:
  ...
  trait _methods[A<:RAW] extends Ordered.zMethodsBase[A] with z.toPrimitives[A]:
    extension(inline x: A)
      ...
      @tn("multiply")       inline def * (inline v: A|Byte|Short|Int|Long)                : A           = (x.real * v.cast[Long]).cast[A]
      @tn("divide")         inline def / (inline v: A|Byte|Short|Int|Long)                : A           = (x.real / v.cast[Long]).cast[A]
      @tn("plus")           inline def + (inline v: A|Byte|Short|Int|Long)                : A           = (x.real + v.cast[Long]).cast[A]
      @tn("minus")          inline def - (inline v: A|Byte|Short|Int|Long)                : A           = (x.real - v.cast[Long]).cast[A]
      @tn("remainder")      inline def % (inline v: A|Byte|Short|Int|Long)                : A           = (x.real % v.cast[Long]).cast[A]
      @tn("greater")        inline def > (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >  v.cast[Double]; case _ :Float => x.real >  v.cast[Float]; case _ => x.real >  v.cast[Long] }
      @tn("greaterOrEqual") inline def >=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >= v.cast[Double]; case _ :Float => x.real >= v.cast[Float]; case _ => x.real >= v.cast[Long] }
      @tn("less")           inline def < (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <  v.cast[Double]; case _ :Float => x.real <  v.cast[Float]; case _ => x.real <  v.cast[Long] }
      @tn("lessOrEqual")    inline def <=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <= v.cast[Double]; case _ :Float => x.real <= v.cast[Float]; case _ => x.real <= v.cast[Long] }
