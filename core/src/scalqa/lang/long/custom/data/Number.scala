...
import java.lang.{ Long as NUMBER }

abstract class Number[A<:Opaque.Long](typeName:String) extends Ordered[A](typeName):

  @fast given math: G.Math[A] = Lang.Long.math.cast[G.Math[A]]

  extension[A<:RAW](inline x: A)
    /**/                        inline def abs                                                : A           = {val v=x; if(v.real >= 0) v else (-v.real).cast[A]}
    /**/                        inline def unary_-                                            : A           = (- x.real).cast[A]
    @tn("numberMultiply")       inline def * (inline v: A|Byte|Short|Int|Long)                : A           = (x.real * v.cast[Long]).cast[A]
    @tn("numberDivide")         inline def / (inline v: A|Byte|Short|Int|Long)                : A           = (x.real / v.cast[Long]).cast[A]
    @tn("numberPlus")           inline def + (inline v: A|Byte|Short|Int|Long)                : A           = (x.real + v.cast[Long]).cast[A]
    @tn("numberMinus")          inline def - (inline v: A|Byte|Short|Int|Long)                : A           = (x.real - v.cast[Long]).cast[A]
    @tn("numberRemainder")      inline def % (inline v: A|Byte|Short|Int|Long)                : A           = (x.real % v.cast[Long]).cast[A]
    @tn("numberGreater")        inline def > (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >  v.cast[Double]; case _ :Float => x.real >  v.cast[Float]; case _ => x.real >  v.cast[Long] }
    @tn("numberGreaterOrEqual") inline def >=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >= v.cast[Double]; case _ :Float => x.real >= v.cast[Float]; case _ => x.real >= v.cast[Long] }
    @tn("numberLess")           inline def < (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <  v.cast[Double]; case _ :Float => x.real <  v.cast[Float]; case _ => x.real <  v.cast[Long] }
    @tn("numberLessOrEqual")    inline def <=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <= v.cast[Double]; case _ :Float => x.real <= v.cast[Float]; case _ => x.real <= v.cast[Long] }
