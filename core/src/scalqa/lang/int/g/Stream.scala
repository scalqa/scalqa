...
abstract class Stream[A<:Raw] extends Val.Stream[A] with Val.Stream.Custom.Discharge[A] with any.z.PrimitiveTag.Int:
  self =>
...
object Stream:
  ...
  /**/                    def apply    [A<:Raw](v: A*)                       : Stream[A] = v match{ case v: scala.collection.immutable.ArraySeq.ofInt => v.unsafeArray.stream.cast[Stream[A]]; case v => v.stream.raw}
...
___________________________________________________________________________*/
/**
@class Stream -> ### Int Specialized Generic Stream
