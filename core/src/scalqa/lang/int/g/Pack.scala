...
class Pack[A<:Raw] private(_a: Array[Int], sz: Int) extends ><[A] with Idx[A]:
  private def this(a: Array[Int]) = this(a,a.length)
  ...
  /**/                   def compact                  : Pack[A]       = {if(ar.length>sz) {ar=new Array[Int](sz).^(ar.copyTo(_,0,0,sz))}; this}
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]       = new Pack(lang.array.z.joinAllAt.int(ar,i,vs.cast[~[Int ]],sz))
  /**/                   def joinAll(vs: ~[A])        : this.type   = vs.read_?.map(v => Pack.Buf(ar,sz+1,v,vs).mk).or(this).cast[this.type]

object Pack:
  /**/                   def fromVarArg[A<:Raw](v: A, vs: Seq[A])      : Pack[A] = new Pack(Array(v.real,vs.cast[Seq[Int]] *))
  /**/            inline def fromArray [A<:Raw](v: Array[Int])         : Pack[A] = fromArray(v,v.length)
  /**/                   def fromArray [A<:Raw](v: Array[Int],sz: Int) : Pack[A] = new Pack(v.copySize(sz),sz)
  /**/                   def fromStream[A<:Raw](v: ~[A])               : Pack[A] = void[A].joinAll(v)
  @tn("getVoid")         def void      [A<:Raw]                        : Pack[A] = zVoid.cast[Pack[A]]; private object zVoid extends Pack(Int.emptyArray) with Gen.Void
  ...
  private class Buf[A<:Raw] private(a: Array[Int],sz:Int) extends Buffer[A](a,sz):
    def this(a: Array[Int], sz: Int, v: A, vs: ~[A]) = { this(a.copySize(vs.size_?.map(sz + _) or sz + J.initSize).^(_(sz-1)=v), sz); addAll(vs) }
    def mk: Pack[A] =
