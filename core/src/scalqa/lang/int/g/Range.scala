...
class Range[A<:Raw](_start: A, _size: Int) extends Val.<>[A] with Able.Size with Able.~[A] with any.z.PrimitiveTag.Int:
  ...
  // -----------------------------------------------------------------------------------------------------------------------------
  @tn("stream")     def ~                      : Stream[A]       = z.range.Stream_fromRange[A](_start.real,endX.real,1L)
  @tn("step_Stream")def step_~(step: Int)      : Stream[A]       = z.range.Stream_fromRange(_start.real,endX.real,step)
...
object Range:
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------
  implicit inline def implicitRequest(v: \/)    : Int.<>  = z_Void;  object z_Void extends G.<>[Int](0,0) with Gen.Void
  extension(x:Int.<>)
    @tn("moveRight")inline def >>(move: Int)    : Int.<>  = Range(x.start+move,x.size)
    @tn("moveLeft") inline def <<(move: Int)    : Int.<>  = Range(x.start-move,x.size)
    @tn("grow")     inline def +(growBy: Int)   : Int.<>  = Range(x.start,x.size + growBy)
    @tn("shrink")   inline def -(shrinkBy: Int) : Int.<>  = Range(x.start,x.size - shrinkBy max 0)
    @tn("checkIn")  inline def checkIn(size:Int): Int.<>  = {if(x.start<0 || x.endX>size) J.illegalArgument(""+x.start+" <>> "+x.endX+" is out of range 0 <>> "+size); x}
    @tn("default")  inline def default(size:Int): Int.<>  = {var v=x; if(v.isInstanceOf[Void]) v = 0 <>> size; v }
