...
abstract class Ordered[A<:Opaque](name: String) extends Data[A](name) with Ordered._methods:
  ...
  @fast given ordering : G.Ordering[A] = Short.ordering.cast[G.Ordering[A]]
...
___________________________________________________________________________*/
/**
@class Ordered -> ### Short Opaque Ordered Data Companion Object Base

  Opaque Ordered Data element is provided with default ordering and can be compared with comparison operators.

  See [sample type definition](https://github.com/scalqa/samples/blob/master/src/example/opaque/all/OrderedShortData.scala).

@trait _methods -> Default methods for all Short.Opaque.Data.Ordered defined types
