...
class Set[A<:Raw] private(real: IntMap[Unit]) extends Val.Set[A] with Collection[A]:
  ...
  /**/     def stream                   : Stream[A] = real.keysIterator.stream.raw.map(_.toShort.cast[A])
  /**/     def join(v: A)               : Set[A]    = new Set(real.updated(v.real.toInt,()))
  /**/     def joinAll(v: Val.Stream[A]): Set[A]    = new Set(real.concat(v.map(v => (v.real.toInt,())).iterator))
...
___________________________________________________________________________*/
/**
@class Set -> ### Short Specialized Generic Set
