...
class Pack[A<:Raw] private(_a: Array[Short], sz: Int) extends ><[A] with Idx[A]:
  ...
  /**/                   def compact                  : Pack[A]       = {if(ar.length>sz) {ar=new Array[Short](sz).^(ar.copyTo(_,0,0,sz))}; this}
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]       = new Pack(lang.array.z.joinAllAt.short(ar,i,vs.cast[~[Short]],sz))
  /**/                   def joinAll(vs: ~[A])        : this.type   = vs.read_?.map(v => Pack.Buf(ar,sz+1,v,vs).mk).or(this).cast[this.type]

object Pack:
  /**/                   def fromVarArg[A<:Raw](v: A, vs: Seq[A])      : Pack[A] = new Pack(Array(v.real,vs.cast[Seq[Short]] *))
  /**/            inline def fromArray [A<:Raw](v: Array[Short])       : Pack[A] = fromArray(v,v.length)
  /**/                   def fromArray [A<:Raw](v: Array[Short],sz:Int): Pack[A] = new Pack(v.copySize(sz),sz)
  /**/                   def fromStream[A<:Raw](v: ~[A])               : Pack[A] = void[A].joinAll(v)
  @tn("getVoid")  inline def void      [A<:Raw]                        : Pack[A] = zVoid.cast[Pack[A]]; object zVoid extends Pack(Short.emptyArray) with Gen.Void
