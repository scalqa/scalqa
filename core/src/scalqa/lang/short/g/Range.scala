...
class Range[A<:RAW](_start: A, _size: Int) extends Val.<>[A] with Able.Size with Able.~[A] with any.raw.Specialized.OnShort:
  ...
  /**/              def ordering               : G.Ordering[A] = Short.ordering.cast[G.Ordering[A]]
  @tn("stream")     def ~                      : Stream[A]     = Z.Stream_fromRange(_start.cast[Int],endX.real,1)
  @tn("step_Stream")def step_~(step: Int)      : Stream[A]     = Z.Stream_fromRange(_start.cast[Int],endX.real,step)
