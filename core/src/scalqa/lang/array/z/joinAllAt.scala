...
object joinAllAt:
  ...
  def ref    (x:Array[Ref],    i:Int, v: ~[Ref],    sz:Int): Array[Ref]    ={val b=lang.any.ref  .Buffer.accessible(new Array[Ref    ](v.size_?.map(_ + sz) or i+J.initSize),i).^(_ ++= v); var a=b.access; val s=b.size+sz-i; if(a.length != s) a=a.copySize(s); x.copyTo(a,0,0,i); x.copyTo(a,b.size,i,sz-i); a}
