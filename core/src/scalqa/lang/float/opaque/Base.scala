...
abstract class Base[A<:Opaque](name:String) extends Any.Opaque.Base[A](name,ClassTag.Float.cast[ClassTag[A]]) with Base._methods with gen.`given`.VoidDef.FloatRaw[A]:
