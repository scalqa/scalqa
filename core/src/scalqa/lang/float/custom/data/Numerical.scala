...
abstract class Numerical[A<:Opaque.Float](typeName:String) extends Ordered[A](typeName) with Numerical._methods[A]:
...
object Numerical:
  ...
  trait _methods[A<:RAW] extends Ordered.zMethodsBase[A] with z.toPrimitives[A]:
    extension(inline x: A)
      ...
      @tn("multiply")       inline def * (inline v: A|Byte|Short|Int|Long|Float)          : A           = (x.real * v.cast[Float]).cast[A]
      @tn("divide")         inline def / (inline v: A|Byte|Short|Int|Long|Float)          : A           = (x.real / v.cast[Float]).cast[A]
      @tn("plus")           inline def + (inline v: A|Byte|Short|Int|Long|Float)          : A           = (x.real + v.cast[Float]).cast[A]
      @tn("minus")          inline def - (inline v: A|Byte|Short|Int|Long|Float)          : A           = (x.real - v.cast[Float]).cast[A]
      @tn("remainder")      inline def % (inline v: A|Byte|Short|Int|Long|Float)          : A           = (x.real % v.cast[Float]).cast[A]
      @tn("greater")        inline def > (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >  v.cast[Double]; case _ => x.real >  v.cast[Float] }
      @tn("greaterOrEqual") inline def >=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real >= v.cast[Double]; case _ => x.real >= v.cast[Float] }
      @tn("less")           inline def < (inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <  v.cast[Double]; case _ => x.real <  v.cast[Float] }
      @tn("lessOrEqual")    inline def <=(inline v: A|Byte|Short|Int|Long|Float|Double)   : Boolean     = inline v match{ case _ :Double => x.real <= v.cast[Double]; case _ => x.real <= v.cast[Float] }
