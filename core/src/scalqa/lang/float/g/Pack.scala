...
class Pack[A<:Raw] private(_a: Array[Float], sz: Int) extends ><[A] with Idx[A]:
  private def this(a: Array[Float]) = this(a,a.length)
  ...
  /**/                   def compact                  : this.type   = {if(ar.length>sz) {ar=new Array[Float](sz).^(ar.copyTo(_,0,0,sz))}; this}
  ...
  /**/                   def joinAllAt(i:Int,vs: ~[A]): Pack[A]     = new Pack(lang.array.z.joinAllAt.float(ar,i,vs.cast[~[Float]],sz))
  /**/                   def joinAll(vs: ~[A])        : Pack[A]     = vs.read_?.map(v => Pack.Buf(ar,sz+1,v,vs).mk) or this

object Pack:
  /**/                   def fromVarArg[A<:Raw](v: A, vs: Seq[A])      : Pack[A] = new Pack(Array(v.real,vs.cast[Seq[Float]] *))
  /**/            inline def fromArray [A<:Raw](v: Array[Float])       : Pack[A] = fromArray(v,v.length)
  /**/                   def fromArray [A<:Raw](v: Array[Float],sz:Int): Pack[A] = new Pack(v.copySize(sz),sz)
  /**/                   def fromStream[A<:Raw](v: ~[A])               : Pack[A] = void[A].joinAll(v)
  @tn("getVoid")  inline def void      [A<:Raw]                        : Pack[A] = zVoid.cast[Pack[A]]; object zVoid extends Pack(Float.emptyArray) with Gen.Void
  ...
  private class Buf[A<:Raw] private(a: Array[Float],sz:Int) extends Buffer[A](a,sz):
    def this(a: Array[Float], sz: Int, v: A, vs: ~[A]) = { this(a.copySize(vs.size_?.map(sz + _) or sz + J.initSize).^(_(sz-1)=v), sz); addAll(vs) }
    def mk: Pack[A] =
