...
object  Z:
  ...
  object VoidStream extends G.~[Char] with Able.Size.Zero with Gen.Void:
    @tn("readRaw_Opt")  def readRaw_?  : Char.Opt = \/
  ...
  class Stream_fromRange[A<:Raw](start: Int, endX: Int, step: Int) extends G.~[A] with Able.Size with Able.Doc:
    ...
    /**/               def doc       : Doc      = this.defaultDoc ++= (i<endX) ? ("from",i.toChar.toString) += ("step",step)
  ...
  object OptEmptyDef extends Any.Def.Void[G.Opt[Char]] with Any.Def.Empty[G.Opt[Char]]:
    def value_isVoid( v:G.Opt[Char]) = v.isEmpty
    def value_isEmpty(v:G.Opt[Char]) = v.isEmpty
