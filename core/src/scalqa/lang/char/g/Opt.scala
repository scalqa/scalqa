...
object Opt extends z.opt._base:
  ...
  extension[A<:Raw](x: Opt[A])
    /**/               inline def ref                                         : Val.Opt[A] = (if(x.isEmpty) ZZ.None else java.lang.Character.valueOf(x.real.toChar)).cast[Val.Opt[A]]
  ...
  object OPAQUE:
    opaque type TYPE[+A<:Raw] <: Int.Opaque = Int.Opaque
