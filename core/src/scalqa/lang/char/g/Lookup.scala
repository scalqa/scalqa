...
object Lookup:
  ...
  class Stable[A<:Raw,B] private(real: IntMap[B]) extends Val.Lookup.Stable[A,B] with Lookup[A,B]:
    ...
    override def keyStream                   : Stream[A]          = real.keysIterator.stream.raw.map(_.toChar.cast[A])
    override def pairStream                  : Val.Stream[(A,B)]  = real.stream.map(v => (v._1.toChar.cast[A],v._2))
...
___________________________________________________________________________*/
/**
@trait Lookup -> ### Char Specialized Generic Lookup Collection
