...
class Buffer[A<:Raw](arrayToUse: Array[Double], sizeToStartWith: Int) extends Val.Buffer[A] with Idx.Mutable[A]:
  ...
  private               var ar                              : Array[Double]  = arrayToUse
  ...
  /**/                  def size                            : Int            = sz
  @tn("stream")override def ~                               : Stream[A]      = lang.array.z.As.DoubleStream(ar,sz)
...
object Buffer:
  ...
  implicit inline def implicitFrom[A<:Raw](inline v: NEW)                  : Buffer[A]                              = new Buffer()
