...
abstract class Stream[A<:Raw] extends Val.Stream[A] with Val.Stream.Custom.Discharge[A] with any.z.PrimitiveTag.Double:
  self =>
...
object Stream:
  ...
  /**/                    def apply    [A<:Raw](v: A*)                       : Stream[A] = v match{ case v: scala.collection.immutable.ArraySeq.ofDouble => v.unsafeArray.stream.cast[Stream[A]]; case v => v.stream.raw}
...
___________________________________________________________________________*/
/**
@class Stream -> ### Double Specialized Generic Stream
