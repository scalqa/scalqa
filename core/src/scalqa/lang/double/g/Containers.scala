...
transparent trait Containers[A<:Raw]:
  ...
  /**/          type Lookup[B]    = Val.Lookup[A,B];                  val Lookup     = REF.Z.Lookup    .cast[REF.Lookup[A]]
  ...
  /**/          type Ordering     = G.Ordering[A];                    val Fun        = RAW.Z.Fun       .cast[RAW.Fun[A]]
  /**/          type Set          = Val.Set[A];                       def Set        = REF.Z.Set       .cast[REF.Set[A]]
...
___________________________________________________________________________*/
/**
@trait Containers -> ### Double Generic Type Assigened Containers

     This trait customizes generic containers with given Primitive type.

     G.Containers trait is already mixed with Opaque.Data, so the application code usually uses them indirectly when defining data elements.
