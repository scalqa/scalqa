...
object Deque:
  ...
  class Entry(val value: Any, val next: Entry) extends Able.Info:
    ...
    def info     : Info    = Info("Entry:"+this.^.hash) += ("thisOk",ok) += ("nextOk",next.ok) += ("priorOk",prior.ok) += ("size",size) += ("value",value.toString)
  ...
  object Root extends Entry(null, null):
    ...
    override def info  = Info("Entry:Void")
