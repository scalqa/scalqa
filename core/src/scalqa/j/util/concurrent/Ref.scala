...
import java.util.concurrent.atomic.{AtomicReference as ATOM}
import Concurrent.Ref

object Ref:
  def apply[A](v: A) = new ATOM(v).asOpaque[Ref[A]]

  extension[A](x: Ref[A])
    private inline def real= x.cast[ATOM[A]]
  ...
  object opaque:
    opaque type `type`[A] <: Opaque.Ref = ATOM[A] & Opaque.Ref
