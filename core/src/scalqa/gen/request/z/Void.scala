...
private[request] object Void:
  object Ordering            extends Ordering[Any]                   with Void { def compare(x: Any, y: Any) = 0 }
  object Filter              extends (Any => Boolean)                with Void { def apply(v: Any) = true }
  object ToStringFunction    extends (Any => String)                 with Void { def apply(v: Any) = "" }
  object Function0           extends (() => Unit)                    with Void { def apply = () }
  object Function1           extends ((Any) => Unit)                 with Void { def apply(a: Any) = () }
  object Function1_Opt       extends ((Any) => Opt[Any])             with Void { def apply(a: Any) : Opt[Any] = \/ }
  object Function1_Any       extends ((Any) => Any)                  with Void { def apply(a: Any) = J.unsupportedOperation() }
  ...
  object JavaList            extends java.util.AbstractList[Nothing] with Void { def get(i: Int) = J.illegalArgument(); def size = 0 }
  object Exception           extends Exception("void")               with Void
  object IntFunToDouble      extends Int.Fun.ToDouble                with Void { def apply(v:Int) = 0.0 }
  object IntFunToLong        extends Int.Fun.ToLong                  with Void { def apply(v:Int) = 0L  }

  object Math                extends Gen.Math[\/] with Void with ~~.Custom.Math.Average[\/]:
