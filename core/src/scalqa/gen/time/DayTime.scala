...
object DayTime extends Long.Opaque.Data.Ordered[DayTime]("DayTime") with x.Nanos.Base.Length:
  ...
  implicit       inline def implicitFrom(inline v:Length)      : DayTime = v.nanosTotal.toOpaque
  implicit       inline def implicitFrom(v: \/)                : DayTime = 0L.toOpaque
  implicit       inline def implicitFrom(v: CURRENT)           : DayTime = current
