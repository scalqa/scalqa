...
object OptShape extends zOptShapeDefault:
  ...
  inline given givenRef     [A<:Lang.AnyRef]: OptShape[A,      Val.Opt[A]] = new OptShape[A,       Val.Opt[A]]
...
private[`given`] class zOptShapeDefault:
  inline given givenAny      [A             ]: OptShape[A,      Val.Opt[A]] = new OptShape[A,       Val.Opt[A]]
