...
transparent trait _extension:
  ...
  extension[A](x: Ordering[A])
    ...
    @tn("voidFirst_View") def voidFirst_^        (using Given.VoidTag[A]) : Ordering[A]      = z.View.VoidPositionFirst[A](x)
    @tn("voidLast_View")  def voidLast_^         (using Given.VoidTag[A]) : Ordering[A]      = z.View.VoidPositionLast[A](x)
