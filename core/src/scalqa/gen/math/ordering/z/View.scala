...
object View:
  ...
  class VoidPositionFirst[A :Given.VoidTag](val real: Comparator[A]) extends Ordering[A]:
    def compare(x: A, y: A): Int = if (x.^.isVoid) { if (y.^.isVoid) 0 else 1 } else if (y.^.isVoid) -1 else real.compare(x, y)

  class VoidPositionLast[A :Given.VoidTag](val real: Comparator[A]) extends Ordering[A]:
    def compare(x: A, y: A): Int = if (x.^.isVoid) { if (y.^.isVoid) 0 else -1 } else if (y.^.isVoid) 1 else real.compare(x, y)
