...
trait Ordering[A] extends scala.math.Ordering[A]:
  ...
  /**/                  def join(c: ORD[A])                     : ORD[A]      = if (this.^.isVoid) c else if (c.^.isVoid) this else z.View.Joined[A](this, c)
  ...
  @tn("voidFirst_View") def voidFirst_^(using Given.VoidDef[A]) : ORD[A]      = z.View.VoidPositionFirst[A](this)
  @tn("voidLast_View")  def voidLast_^ (using Given.VoidDef[A]) : ORD[A]      = z.View.VoidPositionLast[A](this)
