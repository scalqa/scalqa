...
abstract class Rounding private[math](val mode: java.math.RoundingMode, private[scalqa] val bigDecimalRounding: Int):
  ...
  def decimal(v:Double,scale: Int)   : Double   = { val l = v.Long
...
object Rounding:
  ...
  implicit inline def implicitRequest(inline v: DEFAULT): Rounding = HalfUp
  implicit inline def implicitRequest(inline v: UP)     : Rounding = Up
  implicit inline def implicitRequest(inline v: DOWN)   : Rounding = Down
